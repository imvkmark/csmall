...
require WSLM_ROOT.'languages/'.LANG.'/wslm_admin.lang.php';
...
elseif($file != 'login')
{
	showmessage('请登录！', '?mod=wslm&file=login&forward='.urlencode(URL),1,1);
}
...
if($mod != 'wslm' && !@include WSLM_ROOT.$M['path'].'admin/admin.inc.php') showmessage('The file ./'.$M['path'].'admin.inc.php is not exists!');
if(!@include WSLM_ROOT.(isset($M['path']) ? $M['path'] : '').'admin/'.$file.'.inc.php') showmessage("The file ./{$M['path']}admin/{$file}.inc.php is not exists!");
?>