...
switch($action)
{
    case 'add':
		...
		if($CATEGORY[$catid]['child'])
		{
			...
			{
				$categorys = form::select_category('wslm', 0, 'catid', 'catid', '请选择栏目', $catid, 'size="2" style="height:260px;width:350px;"', 1, 1);
			}
		...
		else
		{
			if($dosubmit)
			{
				...
				if($contentid && isset($MODULE['pay']) && $presentpoint && $info['status']==99)
				{
					...
					$pay_api->update_exchange('wslm', 'point', $presentpoint, $api_msg);
				}
			...
			else
			{
				...
				$head['title'] = $CATEGORY[$catid]['catname'].'_发布信息_'.$WSLM['sitename'];
    ...
    case 'edit':
		...
		else
		{
			...
			$forminfos = $content_form->get($data);
			$position = catpos($catid, SCRIPT_NAME.'?action=manage&catid={$catid}');
			$head['title'] = $CATEGORY[$catid]['catname'].'_修改信息_'.$WSLM['sitename'];
			include template($mod, 'manage');
		}
		break;
	...
	case 'preview':
		...
		$info = $out->get($data);
		$fields = $out->fields;
		$head['title'] = $data['title'].'_预览信息_'.$WSLM['sitename'];
		include template($mod, 'manage');
		break;
	...
	default :
		...
		$position = catpos($catid, SCRIPT_NAME.'?action=manage&catid={$catid}');
		$head['title'] = $catid ? $CATEGORY[$catid]['catname'].'_信息管理_'.$WSLM['sitename'] : '信息管理_'.$WSLM['sitename'];
		include template($mod, 'manage');
}
?>