...
<table class="table_form" cellspacing="1" cellpadding="0">
   ...
   <tr> 
    ...
    <tr> 
      ...
      <td>
<?=form::select_template('link', 'tag_config[template]', 'template', $template, '','tag_link')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="$('#template_edit').show();$('#template_data').load('?mod=wslm&file=template&action=data_ajax&module=<?=$module?>&template='+$('#template').val())"> 【注:只能修改非默认模板】
      </td>
    </tr>
