...
<?=form::select_type('link', 'typeid','','',$typeid)?> 【为0则调用所有】
    ...
    <tr> 
      <td><b>此标签调用的模板</b></td>
      <td >
<?=form::select_template('link', 'tag_config[template]', 'template', $template, '','tag_link')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=wslm&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'">【注:只能修改非默认模板】
      </td>
    </tr>
	<tr> 
