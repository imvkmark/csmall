...
switch ($action)
{
	default:
		foreach ($CATEGORY as $key=>$val)
		{
			...
			$categories[$key] = array('id'=>$val['catid'],'parentid'=>$val['parentid'],'name'=>$val['catname'],'day'=>$count->get_count($val['catid'], 'day') ? $count->get_count($val['catid'], 'day') : 0,'yestoday'=>$count->get_count($val['catid'], 'yestoday') ? $count->get_count($val['catid'], 'yestoday') : 0, 'week'=>$count->get_count($val['catid'], 'week') ? $count->get_count($val['catid'], 'week') : 0, 'month'=>$count->get_count($val['catid'], 'month') ? $count->get_count($val['catid'], 'month') : 0,'year'=>'<a onmouseover="javascript:$(\'#year_'.$val['catid'].'\').load(\'?mod=wslm&file=count&action=get_year&catid='.$val['catid'].'\')" style="cursor:pointer ">查看</a>', 'all'=>$count->count_all($val['catid']) ? $count->count_all($val['catid']) : 0);
