...
if($_userid && $_groupid == 1 && $_SESSION['is_admin'] == 1) showmessage($LANG['you_are_logined'], '?mod=wslm&file=index&action=index');

if($WSLM['adminaccessip'] && ip_access(IP, $WSLM['adminaccessip'])) showmessage($LANG['visit_banned']);

$code = new times();
$code->set('checkcode', 3600, 1);

if($WSLM['maxloginfailedtimes'])
{
	$times = new times();
	$times->set('login', $WSLM['maxiplockedtime']*3600, $WSLM['maxloginfailedtimes']);
	if($times->check()) showmessage('登录失败超过'.$WSLM['maxloginfailedtimes'].'次！<br />您的IP已经被系统锁定，'.$WSLM['maxiplockedtime'].'小时后将自动解除锁定。', SITE_URL);
}

if($dosubmit)
{
	require WSLM_ROOT.'languages/'.LANG.'/member.lang.php';
	...
	if(!$result)
	{
		if($WSLM['maxloginfailedtimes']) $times->add();
		$code->add();
		showmessage($member->msg(), HTTP_REFERER);
	}
    if($WSLM['maxloginfailedtimes']) $times->clear();
	$code->clear();
	if($result['groupid'] > 1)
	{
        $member->logout();
