...
<table cellpadding="0" cellspacing="1" class="table_form">
	...
	<tr>
		...
		<td><input type="text" id="dbpw" name="setting[dbpw]"  require="<?=($setting[dbhost]=='mysql')? 'true': 'false'?>"  msg="不得为空"  datatype="limit" min="1" max="50" value="<?=$setting[dbpw]?>" size="20" onblur="if($('#dbtype').val() == 'mysql') {$('#database_seleced').load('?mod=wslm&file=import&action=get_database&dbhost='+$('#dbhost').val()+'&dbtype='+$('#dbtype').val()+'&dbuser='+$('#dbuser').val()+'&dbpw='+$('#dbpw').val()+'&dbname=&charset='+$('#charset').val());$('#database_seleced').show();}"></td>
	</tr>
	<tr id="db_charset">
...
<table cellpadding="0" cellspacing="1" class="table_form">
    ...
    <tr>
    	<th class="align_right"><strong>wslm 会员组</strong></th>
...
$(".fields").click(function(){
	...
	else
	{
		...
		data:'mod=wslm&file=import&action=get_fields&dbtype='+$('#dbtype').val()+'&dbhost='+$('#dbhost').val()+'&dbuser='+$('#dbuser').val()+'&dbpw='+$('#dbpw').val()+'&dbname='+$('#dbname').val()+'&charset='+$('#charset').val()+'&tables='+$('#db_tables').val()+'',
		async: false 
		}).responseText;
		if(html!='' && html != 'no')
		{
...
function get_tables()
{
	...
	$.get("?mod=wslm&file=import&action=get_tables",{dbtype:$('#dbtype').val(), dbhost:$('#dbhost').val(), dbuser:$('#dbuser').val(), dbpw:$('#dbpw').val(), dbname:$('#dbname').val(), charset:$('#charset').val()}, function(data){
