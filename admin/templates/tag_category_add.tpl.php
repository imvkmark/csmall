...
<input name="forward" type="hidden" value="?mod=wslm&file=tag&action=manage&module=<?=$module?>&type=<?=$type?>">
<table cellpadding="0" cellspacing="1" class="table_form">
	...
	<tr>
      ...
      <td><input type="text" name="tag_config[module]" id="themodule" size="15" onBlur="$('#category').load('?mod=wslm&file=tag&action=ajax_category&module='+this.value);"> <?=form::select_module('select_module', '', '请选择', '', 'onchange="myform.themodule.value=this.value;$(\'#category\').load(\'?mod=wslm&file=tag&action=ajax_category&module=\'+this.value);"')?></td>
    </tr>
	<tr>
    ...
    <tr>
      ...
      <td><?=form::select_template($module, 'tag_config[template]', 'template', 'tag_category', '', 'tag_category')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=wslm&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'"></td>
    </tr>
    <tr>
