...
<body>
<?php if($catid){ ?>
<div class="pos"><strong>当前栏目</strong>：<a href="?mod=wslm&file=category&action=manage">栏目管理</a><?=catpos($catid, '?mod=wslm&file=category&action=manage&catid=$catid')?></div>
<?php } ?>
...
<tr>
	...
	<td><a href='?mod=wslm&file=category&action=edit&catid=<?=$r['catid']?>&parentid=<?=$r['parentid']?>'><span class='<?=$r['style']?>'><?=$r['catname']?></span></a></td>
	<td class="align_c"><?=$r['type'] == 0 ? '内部栏目' : ($r['type'] == 1 ? '<font color="blue">单网页</font>' : '<font color="red">外部链接</font>')?></td>
	<td class="align_c"><?php if($r['type'] == 0) { ?><a href="?mod=wslm&file=model_field&action=manage&modelid=<?=$r['modelid']?>"><?=$MODEL[$r['modelid']]['name']?></a><?php } ?></td>
