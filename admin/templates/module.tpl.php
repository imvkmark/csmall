...
if(is_array($data)){
	foreach($data as $module){
		$mod_root = WSLM_ROOT.$MODULE[$module['module']]['path'];
?>
<tr>
    <td class="align_l"><a href="?mod=wslm&file=module&action=view&module=<?=$module['module']?>" title="发布日期：<?=$module['publishdate']?><?="\n"?>安装日期：<?=$module['installdate']?>"><?=$module['name']?></a></td>
    ...
    <td class="align_c">
    <a href="?mod=wslm&file=module&action=faq&module=<?=$module['module']?>" title="点击查看使用帮助">帮助</a> | 
    ...
    {
        if($module['disabled'])	echo "<a href=\"?mod=wslm&file=module&action=disable&value=0&module=".$module['module']."\">启用</a> | ";
        else echo "<a href=\"?mod=wslm&file=module&action=disable&value=1&module=".$module['module']."\">禁用</a> | ";
    ?>
    <a href="javascript:if(confirm('确认卸载该模块吗？将会删除该模块所有数据')) location='?mod=wslm&file=module&action=uninstall&module=<?=$module['module']?>'">卸载</a>
    <? } ?>
    </td>
</tr>
<?php 
