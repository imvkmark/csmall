...
<tbody>
    ...
    <tr> 
      ...
      <td><input type="text" name="tag_config[number]" size="10" value="10"> 条 <span id="pagesize" style="display:none;">当此标签用于栏目列表分页时且生成Html时，必须与系统所设置分页数（<font color="red"><?=$WSLM['pagesize']?></font>）保持一致</span></td>
    </tr>
    <tr> 
      <th><strong>标签模板</strong></th>
      <td><?=form::select_template($module, 'tag_config[template]', 'template', 'tag_content', '', 'tag_content')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=wslm&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'"></td>
    </tr>
    <tr> 
