...
<table cellpadding="2" cellspacing="1" class="table_form">
  ...
  <td>
<?=form::select_category('wslm', 0, 'parentid', 'parentid', '无（作为一级栏目）', $catid,'',2)?>
  </td>
  </tr>
     <tr>
...
<tbody id="model" style="display:'block'">
     <tr>
      ...
      <td><?=form::select_model('modelid', 'modelid', '', $modelid, '')?> <a href="?mod=wslm&file=model&action=manage">管理模型</a></td>
    </tr>
</tbody>
	<tr>
	 <th></th>
...
<table cellpadding="2" cellspacing="1" class="table_form">
  ...
  <input type="hidden" name="category[modelid]" value="<?=$modelid?>"> <?=$MODEL[$modelid]['name']?>
    ...
    <tr>
      ...
      <td><input name='category[catname]' type='text' id='catname' size='40' maxlength='50' require="true" datatype="limit|ajax" min="1" max="50" url="?mod=wslm&file=category&action=checkname&parentid=<?=$parentid?>" msg="字符长度范围必须为1到50位|" msgid="msgid1"> <?=form::style('category[style]','')?><span id="msgid1"></span></td>
    </tr>
    <tr>
      <th><font color="red">*</font> <strong>栏目目录</strong></th>
      <td><input name='category[catdir]' type='text' id='catdir' size='20' maxlength='50' require="true" datatype="limit|ajax" min="1" max="50" url="?mod=wslm&file=category&action=checkdir&parentid=<?=$parentid?>" msg="字符长度范围必须为1到50位|"></td>
    </tr>
    <tr>
    ...
    <tr>
     ...
     <tr>
      ...
      <td><?=form::select(cache_read('workflow.php'), 'setting[workflowid]', 'workflowid', $MODEL[$modelid]['workflowid'])?>  <a href="?mod=wslm&file=workflow&forward=<?=urlencode(URL)?>">管理工作流方案</a></td>
    </tr>
	<tr>
    ...
    <tr>
      ...
      <input name='category[url]' type='text' id='domain' value='' size='60' maxlength='60'> 例如：http://soft.wslm.cn
  ...
  <tbody id='Tabs3' style='display:none'>
   ...
   <tbody id='Tabs4' style='display:none'>
    <tr>
      ...
      <td><?=form::select_template('wslm', 'setting[template_category]', 'template_category', $MODEL[$modelid]['template_category'], '','category')?></td>
    </tr>
    <tr>
      <th><strong>列表页模板</strong></th>
      <td><?=form::select_template('wslm', 'setting[template_list]', 'template_list', $MODEL[$modelid]['template_list'], '','list')?></td>
    </tr>
    <tr>
      <th><strong>内容页模板</strong></th>
      <td><?=form::select_template('wslm', 'setting[template_show]', 'template_show', $MODEL[$modelid]['template_show'], '','show')?></td>
    </tr>
    <tr>
      <th><strong>打印页模板</strong></th>
      <td><?=form::select_template('wslm', 'setting[template_print]', 'template_print', $MODEL[$modelid]['template_print'], '','print')?></td>
    </tr>
	<tr>
	...
	<tr>
      ...
      <td><label><input type="radio" name="setting[thumb_enable]" value=1 <?php if($WSLM['thumb_enable']==1) {?>checked<?php } ?>> 是</label> <label><input type="radio" name="setting[thumb_enable]" value=0 <?php if($WSLM['thumb_enable']==0) {?>checked<?php } ?>> 否</label> </td>
    </tr> 
	<tr>
      <th width='30%'><strong>缩略图大小</strong></th>
      <td><input name='setting[thumb_width]' type='text' id='thumb_width' value='<?=$WSLM['thumb_width']?>' size='5' maxlength='5'> X <input name='setting[thumb_height]' type='text' id='thumb_height' value='<?=$WSLM['thumb_width']?>' size='5' maxlength='5'> px </td>
    </tr>
	<tr>
      <th width='30%'><strong>是否启用水印</strong></th>
      <td><label><input type="radio" name="setting[watermark_enable]" value=1 <?php if($WSLM['watermark_enable']==1) {?>checked<?php } ?>> 是</label> <label><input type="radio" name="setting[watermark_enable]" value=0 <?php if($WSLM['watermark_enable']==0) {?>checked<?php } ?>> 否</label> <span style="color:#ff0000">水印的详细设置遵照网站配置下的附件设置里的设置。如果<a href='?mod=wslm&file=setting&tab=4'>修改</a>将全站生效。</span></td>
    </tr>
  </tbody>
  <tbody id='Tabs5' style='display:none'>
	...
	<tr>
	  ...
	  <a href="?mod=wslm&file=urlrule&action=add&module=wslm&filename=category&forward=<?=urlencode(URL)?>" style="color:red">点击新建URL规则</a></th>
      <td><div id="div_category_urlruleid"></div></td>
    </tr>
	<tr>
      <th><strong>内容页URL规则</strong><br />
	  <a href="?mod=wslm&file=urlrule&action=add&module=wslm&filename=show&forward=<?=urlencode(URL)?>" style="color:red">点击新建URL规则</a></th>
...
<table cellpadding="2" cellspacing="1" class="table_form">
  ...
  <input type="hidden" name="category[parentid]" value="<?=$parentid?>"> <?=$parentid ? $CATEGORY[$parentid]['catname'] : '无'?>
    ...
    <tr>
      ...
      <td><input name='category[image]' type='text' id='image' size='40' maxlength='50'> <?=file_select('image', $catid, 1)?>
    ...
    <tr>
      ...
      <td><?=form::select_template('wslm', 'setting[template]', 'template', 'page', '','page')?></td>
    </tr>
	<tr>
      <th><strong>是否生成Html</strong></th>
      <td>
	  <input type='radio' name='setting[ishtml]' value='1' <?php if($WSLM['ishtml']){ ?>checked <?php } ?> onClick="$('#div_category_urlruleid').load('?mod=<?=$mod?>&file=<?=$file?>&action=urlrule&ishtml=1&category_urlruleid=0');"> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='setting[ishtml]' value='0' <?php if(!$WSLM['ishtml']){ ?>checked <?php } ?> onClick="$('#div_category_urlruleid').load('?mod=<?=$mod?>&file=<?=$file?>&action=urlrule&ishtml=0&category_urlruleid=0');"> 否
	  </td>
    </tr>
	<tr>
