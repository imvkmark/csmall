...
function tag_pop(mod, action, func)
{
	...
	else
	{
		if(func == '')
		{
			url = '?mod=wslm&file=template&action=gettag&operate='+action+'&job=edittemplate&tagname='+tagname;
		}
		else
		{
			url = '?mod=wslm&file=template&action=gettag&function='+action+'&job=edittemplate&tagname='+tagname;	
		}
	}
  ...
  <caption>修改模板</caption>
	...
	<tr>
		...
		<td>
			...
			<input type="button" value="template" style="width:70px" onClick="javascript:if(this.value != '') insertText('<!--{template \'wslm\',\'header\'}-->\n')" />
			<input type="button" value="include" style="width:65px" onClick="javascript:if(this.value != '') insertText('<!--{include WSLM_ROOT.\'*.php\'}-->\n')" />
		</td>
	</tr>
	<tr>
...
?>
            ...
            <input type="text" name="tagname" value="请输入标签名" size="15" onClick="if(this.value == '请输入标签名') this.value=''">&nbsp;<input name="dosubmit" type="button" value="编辑标签" onClick="window.open('?mod=wslm&file=template&action=gettag&operate=edit&job=edittemplate&tagname='+myform.tagname.value,'tag','height=500,width=700,,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no')">&nbsp;&nbsp;
			<input type="button" name="tagpreview" value="预览选中的标签" style="background:blue;color:#ffffff;width:110px" onClick="javascript:if(document.selection.createRange().text != '') tag_pop('wslm','preview','')">
			<input type="button" name="tagedit" value="编辑选中的标签" style="background:blue;color:#ffffff;width:110px" onClick="javascript:if(document.selection.createRange().text != '') tag_pop('wslm','edit','')">
			<input type="button" name="listtag" value="列出模板中的标签" style="background:blue;color:#ffffff;width:110px" onClick="window.open('?mod=wslm&file=tag&action=listtag&job=edittemplate&module='+myform.module.value+'&templatename='+myform.templatename.value+'&template='+myform.template.value,'tag','height=500,width=700,,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no')">
		</td>
	</tr>
</table>
<table cellpadding="0" cellspacing="1" class="table_form">
...
function ShowTab(id)
{
	...
	if(tab == '#tab1')
	{
		$('#tag').hide();
		var select = '';
		$.get('?mod=<?=$mod?>&file=<?=$file?>', {action:'showvar', strvar:'wslm'}, function(data){
			if(data == 'null')
			{
				$('#var').hide();
				return false;	
	...
	{
		$('#var').hide();
		$.get('?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>', {action:'showtags', module:'wslm'}, function(data){
			if(data == 'null')
			{
				$('#tag').hide();
				return false;
...
<table cellspacing="1" cellpadding="0" class="table_list">
    <tr>
    	<td style="">
        	<span id="tab0">
				<?=form::select($modname, 'selectmodule', 'selectmodule', 'wslm', 1, '', 'style="width:150px; margin:5px 0;"')?><br />
            ...
            <span id="tab1" style="display:none;">
				<?=form::select($variable, 'variable', 'variable', 'WSLM', '', '', 'style="width:150px; margin:5px 0;"')?><br />
...
{get sql="select * from wslm_content where contentid=1" /}<br />
...
{get sql="select * from wslm_content where catid=1 and status=99 order by updatetime desc" rows="10"}<br />
 &nbsp;&nbsp;&nbsp;&nbsp;标题：{str_cut($r[title], 50)} URL：{$r[url]} 更新日期：{date('Y-m-d', $r[updatetime])} <br />
{/get}<br />

<span style="color:blue">3、带分页，示例（调用栏目ID为1通过审核的10条信息，标题长度不超过25个汉字，显示更新日期，带分页）：</span><br />
{get sql="select * from wslm_content where catid=1 and status=99 order by updatetime desc" rows="10" page="$page"}<br />
 &nbsp;&nbsp;&nbsp;&nbsp;标题：{str_cut($r[title], 50)} URL：{$r[url]} 更新日期：{date('Y-m-d', $r[updatetime])} <br />
{/get}<br />
分页：{$pages}<br />

<span style="color:blue">4、自定义返回变量，示例（调用栏目ID为1通过审核的10条信息，标题长度不超过25个汉字，显示更新日期，返回变量为 $v）：</span><br />
{get sql="select * from wslm_content where catid=1 and status=99 order by updatetime desc" rows="10" return="v"}<br />
...
function get_db_source()
{
	...
	$.getJSON('?mod=wslm&file=template&action=get_db_source',function(data){
		if(data)
		{
...
function select_db_table(obj)
{
	...
	if(obj!='')
	{
		$.getJSON('?mod=wslm&file=template&action=get_ajax_db_table&name='+obj,function(data){
			if(data)
			{
				$.each(data,function(i,n){
                    var selected = '';
...
function get_fields(val)
{
	...
	if(val!='')
	{
		$.getJSON('?mod=wslm&file=template&action=get_fields&name='+db_source+'&tables='+val,function(data){
			if(data)
			{
				$.each(data,function(i,n){
	...
	$('#template_save').click(function (){
		...
		$.getJSON("?mod=wslm&file=template&action=gettemplate", {filename:filename, page:1}, function (data) {
			if(data != null)
			{
				var str = '<li></li>';
				$.each(data, function(i, n){
	...
	{
		var filename = "<?=$filename?>";
		$.getJSON("?mod=wslm&file=template&action=gettemplate", {filename:filename, page:page}, function (data) {
			if(data != null)
			{
				var str = '<li></li>';
				$.each(data, function(i, n){
	...
	{
		$.post("?mod=wslm&file=template&action=gettemplatedata", {id:id}, function (data){
			if(data != '')
			{
				$('#txt_main').text(data);
			}
