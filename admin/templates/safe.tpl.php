...
<form action="?mod=wslm&file=safe&action=setting" target="dosubmit" method="POST" onsubmit="return postupdate()">
...
{
	showmsgs('正在进行文件类型筛选...');
	$.get('?mod=wslm&file=safe&action=scan_file_type',function(data){
		if(data == 'ok'){
		showmsgs('文件类型筛选完成。');
		scan_file_md5();
		}
...
function scan_file_md5()
{
	...
	$.get('?mod=wslm&file=safe&action=scan_file_md5',function(data){
		if(data == 'ok'){
		showmsgs('文件修改历史对比完成。');
		scan_backdoor();
		}
...
{
	showmsgs('正在进行特征函数扫描...');
	$.get('?mod=wslm&file=safe&action=scan_func',function(data){
		if(data == 'ok'){
		showmsgs('特征函数扫描完成。');
		scan_code();
		}
...
{
	showmsgs('正在进行特征代码扫描...');
	$.get('?mod=wslm&file=safe&action=scan_code',function(data){
		if(data == 'ok'){
		showmsgs('特征代码扫描完成。');
		scan_table();
		}
...
{
	showmsgs('正在为您准备扫描报表...');
	setTimeout('location.href="?mod=wslm&file=safe&action=scan_table"',3000);
}
