...
<tr>
      <th width="35%"><strong>启用附件FTP上传功能</strong><br>开启附件FTP功能后，wslm将采用ftp方式上传附件<br/>
    ...
    <tr>
      <th><strong>Wslm目录</strong><br>有很多虚拟主机的FTP根目录与Web根目录不一样<br/>您需要正确设置才能使用FTP功能</th>
	  ...
	  <br/>注意以“/”结尾，例如有的是 /wwwroot/ 或者 /www/<br/>留空则表示ftp根目录与wslm根目录路径相同</td>
    </tr>
    <tr>
   ...
   <tbody id='Tabs5' style='display:none'>
	...
	<tr>
      <th width="35%"><strong>启用FTP功能</strong><br>开启FTP功能后，wslm将采用ftp方式建立目录和修改权限<br/>
     <?php if(!function_exists('ftp_connect')){ ?><font color="red">当前PHP环境不支持FTP功能！</font><?php }?>
	  </th>
      <td>
  ...
  <tbody id='Tabs7' style='display:none'>
    ...
    <tr>
      <th width="35%"><strong>是否启用正向通行证</strong><br>以WSLM作为整合的服务器端</th>
	...
	<tr>
	  ...
	  <?php
	  $passports = glob(WSLM_ROOT.'member/api/passport_server_*.php');
    ...
    <tr>
      <th><strong>是否启用反向通行证</strong><br>以WSLM作为整合的客户端</th>
...
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
  <tr>
     <td width="35%"><input name='setting[version]' type='hidden' id='version' value='<?=WSLM_VERSION?>'></td>
...
function test_user()
{
	if($('#wslm_username').val() == '')
	{
		alert('请填写帐号');
		$('#wslm_username').focus();
		return false;
	}
	if($('#wslm_password').val() == '')
	{
		alert('请填写密码');
		$('#wslm_password').focus();
		return false;
	}
    $.post('?file=setting&action=test_user', {wslm_username:$('#wslm_username').val(), wslm_password:$('#wslm_password').val()}, function(data){
...
function test_mail()
{
    $.get('?mod=wslm&file=setting&action=test_mail&mail_type='+$('#mail_type').val()+'&mail_server='+$('#mail_server').val()+'&mail_port='+$('#mail_port').val()+'&mail_user='+$('#mail_user').val()+'&mail_password='+$('#mail_password').val()+'&email_to='+$('#email_to').val(), function(data){
		alert(data);
	});
}
...
function test_ftp()
{
    $.post('?mod=wslm&file=setting&action=test_ftp', {ftp_host:$('#ftp_host').val(), ftp_port: $('#ftp_port').val(), ftp_pw:$('#ftp_pw').val(), ftp_user:$('#ftp_user').val(), ftp_path: $('#ftp_path').val()}, function(data){
		alert(data);
	});
}
...
{
    $.post('?mod=wslm&file=setting&action=test_ftp', {ftp_host:$('#upload_ftp_host').val(), ftp_port: $('#upload_ftp_port').val(), ftp_pw:$('#upload_ftp_pw').val(), ftp_user:$('#upload_ftp_user').val(), ftp_path: $('#upload_ftp_path').val()}, function(data){
		alert(data);
	});
}
...
$('#test_uc').click(function () {
	...
	$.ajax({
	 url : '?mod=wslm&file=setting&action=test_uc'+uc,
	 cache : false,
	 timeout: 2000,
	 type : 'GET',
	 async : false,
