...
<tr>
	...
	if(!in_array(1, $admin['roleids']) || !in_array($admin['userid'],$admin_founders)) { ?>
        ...
        <?php } ?>
        <a href="javascript:confirmurl('?mod=wslm&file=admin&action=delete&userid=<?=$admin['userid']?>', '你确认撤销管理员“<?=$admin['username']?>”吗？');">撤消</a>
    <?php }else{ ?>
    <font color="#cccccc">修改 | 锁定 | 撤消</font>
    <?php } ?>
	 | <a href='?mod=wslm&file=log&action=manage&userid=<?=$admin['userid']?>'>日志</a>
    </td>
</tr>
<?php 
	}
