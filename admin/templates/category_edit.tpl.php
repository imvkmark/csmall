...
<table cellpadding="2" cellspacing="1" class="table_form">
  ...
  <td>
<?=form::select_category('wslm', 0, 'category[parentid]', 'parentid', '无（作为一级栏目）', $parentid,'',2)?>
  </td>
  </tr>
     <tr>
    ...
    <tr>
     ...
     <tr>
      ...
      <td><?=form::select(cache_read('workflow.php'), 'setting[workflowid]', 'workflowid', $workflowid)?>  <a href="?mod=wslm&file=workflow&forward=<?=urlencode(URL)?>">管理工作流方案</a></td>
    </tr>
	<tr>
    ...
    <tr>
      ...
      <input name='category[url]' type='text' id='domain' value='<?php if(preg_match('/:\/\//',$url) && substr_count($url, '/')==2) echo $url;?>' size='60' maxlength='60'> 例如：http://soft.wslm.cn
  ...
  <tbody id='Tabs3' style='display:none'>
   ...
   <tbody id='Tabs4' style='display:none'>
    <tr>
      ...
      <td><?=form::select_template('wslm', 'setting[template_category]', 'template_category', $template_category, '','category')?></td>
    </tr>
    <tr>
      <th><strong>栏目列表页模板</strong></th>
      <td><?=form::select_template('wslm', 'setting[template_list]', 'template_list', $template_list, '','list')?></td>
    </tr>
    <tr>
      <th><strong>内容页模板</strong></th>
      <td><?=form::select_template('wslm', 'setting[template_show]', 'template_show', $template_show, '','show')?></td>
    </tr>
	<tr>
      <th><strong>打印页模板</strong></th>
      <td><?=form::select_template('wslm', 'setting[template_print]', 'template_print', $template_print, '','print')?></td>
    </tr>
	<tr>
	...
	<tr>
      ...
      <td><label><input type="radio" name="setting[watermark_enable]" value=1 <?php if($watermark_enable==1) {?>checked<?php } ?>> 是</label> <label><input type="radio" name="setting[watermark_enable]" value=0 <?php if($watermark_enable==0) {?>checked<?php } ?>> 否</label> <span style="color:#ff0000">水印的详细设置遵照网站配置下的附件设置里的设置。如果<a href='?mod=wslm&file=setting&tab=4'>修改</a>将全站生效。</span></td>
    </tr>
	<tr>
...
<table cellpadding="0" cellspacing="1" class="table_form">
  ...
  <td>
<?=form::select_category('wslm', 0, 'category[parentid]', 'parentid', '无（作为一级栏目）', $parentid,'',2)?>  <font color="red">*</font>
  </td>
  </tr>
    <tr>
    ...
    <tr>
      ...
      <td><?=form::select_template('wslm', 'setting[template]', 'template', $template, '','page')?></td>
    </tr>
	<tr>
