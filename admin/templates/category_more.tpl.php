...
?>
<body <?php if($type==1){ ?>onload="$('#div_category_urlruleid').load('?mod=<?=$mod?>&file=<?=$file?>&action=urlrule&ishtml=<?=$WSLM['ishtml']?>&category_urlruleid=0')"<?php } ?> >
<?php if(!isset($type)){ ?>
...
<table cellpadding="2" cellspacing="1" class="table_form">
  ...
  <tr>
    ...
    <td><?=form::select_category('wslm', 0, 'parentid', 'parentid', '无（作为一级栏目）', $parentid, '', 1)?></td>
  </tr>
  <tr>
...
?>
<tr>
    <td class="align_c"><input name='category[catname][<?=$i?>]' type='text' id='catname' size='15' maxlength='20' require="false" datatype="limit|ajax" min="1" max="50" url="?mod=wslm&file=category&action=checkname&parentid=<?=$parentid?>" msg="字符长度范围必须为1到50位|"  mode="2"></td>
    <td class="align_c"><input name='category[catdir][<?=$i?>]' type='text' id='catdir' size='15' maxlength='50' require="false" datatype="limit|ajax" min="1" max="50" url="?mod=wslm&file=category&action=checkdir&parentid=<?=$parentid?>" msg="字符长度范围必须为1到50位|"  mode="2"></td>
