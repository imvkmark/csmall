...
<a href="admin.php?mod=wslm&file=admin&action=view&userid=<?=$userid?>&month=<?=$month?>&year=<?=$i?>">
	<?php
		echo ($year == $i) ? "<font color=red>$i</font> " : $i.' ';?></a>
	<?php }?>
	月份 
	<?php for($i=1;$i<13;$i++) { ?>
	<a href="admin.php?mod=wslm&file=admin&action=view&userid=<?=$userid?>&month=<?=$i?>&year=<?=$year?>"><?php echo ($month == $i) ? "<font color=red>$i</font>" : $i;?></a>
	<?php }?>
	</caption>
	<tr> 
