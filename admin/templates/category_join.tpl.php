...
?>
<body>
<form method="post" action="?mod=wslm&file=category&action=join" name="myform">
...
<th></th>
	<td>
		<input type="submit" id="dosubmit" name="dosubmit" value=" 合 并 " onClick="if(confirm('确认合并栏目吗？')) document.myform.action='?mod=wslm&file=category&action=join';else return false;">
...
<!--
	function check_category(value)
	{
		...
		$.ajax({
			type: "POST",
			url: '?',
			data:'mod=wslm&file=category&action=checkcategory&sourcecatid='+sourcecatid+'&targetcatid='+value,
