...
<table cellpadding="0" cellspacing="1" class="table_info">
  ...
  <tr>
	...
	本功能主要应用于<a href="?mod=wslm&file=template&action=add">get标签调用远程数据库数据</a>或者<a href="?mod=mail&file=maillist&action=get">导出远程数据库邮件列表</a>。<br /><br />
...
$(function(){
    $('#div_tables').load('?mod=wslm&file=datasource&action=tables', {dbhost: '<?=$dbhost?>', dbuser: '<?=$dbuser?>', dbpw: '<?=$dbpw?>', dbname: '<?=$dbname?>', dbcharset: '<?=$dbcharset?>'});
	$('form').checkForm(1);
});
</script>
</body>
