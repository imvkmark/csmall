		...
		if($dosubmit)
		{
			if(!isset($count))
			{
				...
				if(!isset($catids) || $catids[0] == 0) 
				{
					...
					{
						if($v['type'] <= 1 && $v['module']=='wslm') $catids[] = $cid;
					}
				}
			...
			else
			{
				...
				else
				{
					...
					if($page == 1)
					{
						...
						$total = ceil($contents/$WSLM['pagesize']);
				...
				else
				{
					...
					$message = "更新完成！";
					$forward = '?mod=wslm&file=html&action=category';
				}
