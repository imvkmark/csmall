...
switch($action)
{
    case 'manage':
		...
		if($month)
		{
			$logfile = WSLM_ROOT.'data/filterlog/'.$year.$month.'.csv';
		...
		else
		{
			...
			{
				$m = $i > 9 ? $i : '0'.$i;
				$logfile = WSLM_ROOT.'data/filterlog/'.$year.$m.'.csv';
				$result = log_read($logfile, $ip);
    ...
    case 'delete':
		if($month)
		{
			@unlink(WSLM_ROOT.'data/filterlog/'.$year.$month.'.csv');
		}
		else
		{
			for($i=1; $i<=12; $i++)
			{
				$m = $i > 9 ? $i : '0'.$i;
			    @unlink(WSLM_ROOT.'data/filterlog/'.$year.$m.'.csv');
			}
		}
		showmessage('操作成功！', $forward);
		break;
	case 'download':
		file_down(WSLM_ROOT.'data/filterlog/'.$year.$month.'.csv');
		break;
}
?>