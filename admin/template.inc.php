...
$module = isset($module) ? $module : 'wslm';
...
$variable = array('0'=>'请选择类型','WSLM'=>'$WSLM', 'CATEGORY'=>'$CATEGORY', 'MODULE'=>'$MODULE','MODEL'=>'$MODEL', 'member'=>'用户变量');
...
switch($action)
{
	case 'add':
		...
		else
	    {
	    	...
	    	{
	    		if(file_exists(WSLM_ROOT.'templates/'.TPL_NAME.'/'.$m['module'].'/tag_config.inc.php'))
	    		{
	    			$modname[$k] = $m['name'];
	    		}
	    	}
	...
	case 'tag':
		...
		if($mod == 'yp')
		{
			...
			$prowhere = '1';
			$news_where = '1';
			require_once WSLM_ROOT."yp/include/output.func.php";
		}
        ...
        include admin_tpl('block_ajax', 'wslm');
		break;
	...
	case 'manage':
	 ...
	 case 'preview':
		if($dosubmit)
	    {
			...
			$tempfile = WSLM_ROOT.'data/temp/templatepreview.tpl.php';
			file_put_contents($tempfile, $content);
			include $tempfile;
		}
		else
	    {
			$message = '<form name="myform" method="post" action="?mod=wslm&file=template&action=preview&module='.$module.'&template='.$template.'&templatename='.$templatename.'&dosubmit=1"><input type="hidden" name="content"></form><script type="text/javascript">myform.content.value=window.opener.document.myform.content.value;myform.submit();</script>';
			showmessage($message);
		}
		break;
	...
	case 'showtags':
		$tags = include WSLM_ROOT.'templates/'.TPL_NAME.'/'.$module.'/tag_config.inc.php';
	...
	case 'gettag':
		$arr_tag = include WSLM_ROOT.'templates/'.TPL_NAME.'/tag.inc.php';
		...
		$arr_type = include WSLM_ROOT.'templates/'.TPL_NAME.'/'.$module.'/tag_config.inc.php';
	...
	case 'get_db_source':
		$datasource = load('datasource.class.php', 'wslm', 'include/admin');
	...
	case 'get_ajax_db_table':
		...
		$file_table = cache_read(str_replace('MM_LOCALHOST', 'wslm',$name).'.php',WSLM_ROOT.'data/datasource/');
		foreach ($file_table as $key=>$val)
		{
			$key = str_replace('wslm_', DB_PRE, $key);
			$file_table[$key] = $val;
		}
		if(!empty($name) && $name != 'MM_LOCALHOST')
		{
	...
	case 'get_fields':
        header('Content-type: text/html; charset=utf-8');
		$datasource = load('datasource.class.php', 'wslm', 'include/admin');
		$tables_last_name = str_replace(DB_PRE,'wslm_', $tables);
		$file_table = cache_read(str_replace('MM_LOCALHOST', 'wslm',$name).'_'.$tables_last_name.'.php',WSLM_ROOT.'data/datasource/');
		if(!empty($name) && $name!='MM_LOCALHOST')
		{
