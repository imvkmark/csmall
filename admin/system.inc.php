...
$sys_info['php_dns']        = preg_match("/^[0-9.]{7,15}$/", @gethostbyname('www.wslm.cn'));//域名解析
$sys_info['gzip']           = GZIP && function_exists('ob_gzhandler') ;//开启gzip设置

$sys_info['version']        = WSLM_VERSION;
...
$sys_info['web_root']        = WSLM_ROOT;//网站跟目录
//ft min word len
//fopen
$entryarray = array(
            'index.html',
...
{
	global $entryarray;
	$dir = dir(WSLM_ROOT.$directory);
	while($entry = $dir->read())
	{
		if(!in_array($entry, array('.', '..', 'index.htm')))
		{
			if(is_dir(WSLM_ROOT.$directory.'/'.$entry))
			{
