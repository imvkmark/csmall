...
switch($action)
{
	...
	case 'edit':
		...
		if($dosubmit)
		{
			...
			$priv_group->update('catid', $catid, $priv_groupid);
			$priv_role->update('catid', $catid, $priv_roleid);
			showmessage('操作成功！开始更新网站地图...', '?mod=wslm&file=sitemap&dosubmit=1&forward='.urlencode($forward));
		}
		else
	    {
     ...
     case 'delete':
		 ...
		 $cat->delete($catid);
		 showmessage($LANG['operation_success'], '?mod='.$mod.'&file='.$file.'&action=updatecache&forward='.urlencode('?mod=wslm&file=category&action=manage'));
		 break;
	...
	case 'updatecache':
		cache_common();
		cache_category();
		showmessage($LANG['category_cache_update_success'], '?mod=wslm&file=category&action=manage');
		break;
	...
	case 'manage':
		if(count($CATEGORY) < 200)
		{
			...
			foreach($data as $catid => $category)
			{
				 ...
				 $model = $category['type'] == 0 ? '<a href="?mod=wslm&file=model_field&action=manage&modelid='.$category['modelid'].'">'.$MODEL[$category['modelid']]['name'].'</a>' : '';
			...
			$str = "<tr>
						...
						<td class='align_c'><a href='?mod=wslm&file=content&action=manage&catid=\$id' title='管理内容'>\$items</a></td>
	...
	case 'urlrule':
		...
		echo form::select_urlrule('wslm', 'category', $ishtml, 'setting[category_urlruleid]', 'category_urlruleid', $category_urlruleid);
		break;
	case 'show_urlrule':
		$ishtml = intval($ishtml);
		$show_urlruleid = intval($show_urlruleid);
		echo form::select_urlrule('wslm', 'show', $ishtml, 'setting[show_urlruleid]', 'show_urlruleid', $show_urlruleid);
		break;
	case 'checkdir':
    ...
    case 'more':
        if($dosubmit)
		{
			...
			if(!$catid) showmessage('添加失败', '?mod=wslm&file=category&action=more');
	        showmessage('添加成功', '?mod=wslm&file=category&action=more');
		}
		else
	    {
			if(!isset($type))
		...
		if($dosubmit)
		{
			...
			else
			{
				...
				else
				{
					...
					$message = "更新完成！";
					$forward = '?mod=wslm&file=category&action=update_search';
				}
