...
switch($action)
{
    ...
    case 'delete':
		...
		if($result)
		{
			showmessage('操作成功！', '?mod=wslm&file=block&action=manage');
		}
		else
		{
			showmessage('操作失败！');
	...
	case 'index':
		include admin_template('wslm', 'index');
	...
	case 'get_template_example':
		...
		else
		{
			...
			echo form::select($data, 'example', 'example', '', '1', '', 'style="width:100px;" onchange="$(\'#template\').load(\'?mod=wslm&file=block&action=get_template_example&example=\'+this.value)"');
		}
		break;
    ...
    case 'list':
		...
		{
			if($c['type'] == 2 || $c['module'] != 'wslm') continue;
			if($c['type'])
			{
				$type = '单网页';
			...
			else
			{
				...
				$show = "<a href='?mod=wslm&file=content&action=block&catid=$catid&tpl=show'>内容页</a>";
			}
