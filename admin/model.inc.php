...
switch($action)
{
    ...
    case 'edit':
		if($dosubmit)
		{
			...
			if($result)
			{
				...
				$cat = new category('wslm');
				$cat->repair();
				cache_model();

				if(is_array($CATEGORY) && $ishtml != $info['ishtml'])
				{
					$forward = '?mod=wslm&file=url'.$catids.'&forward='.urlencode(URL);
	...
	case 'import':
		if($dosubmit)
		{
			...
			$attachment = new attachment('wslm');
			...
			if(is_array($array['arr_field']) && !empty($array['arr_field']) && $modelid)
			{
				...
				foreach($array['arr_field'] as $arr_field)
				{
					...
					@extract($setting);
					$excutefile = file_get_contents(WSLM_ROOT.'include/fields/'.$formtype.'/field_add.inc.php');
    ...
    case 'disable':
		...
		break;
	case 'urlrule':
		echo $type == 'category' ? form::select_urlrule('wslm', 'category', $ishtml, 'info[category_urlruleid]', 'category_urlruleid', $category_urlruleid) : form::select_urlrule('wslm', 'show', $ishtml, 'info[show_urlruleid]', 'show_urlruleid', $show_urlruleid);
		break;
    default :
}
?>