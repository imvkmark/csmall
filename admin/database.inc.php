...
if(!isset($forward)) $forward = '?mod=wslm&file=database&action=export';
...
switch($action)
{
	...
	case 'import':
		...
		{
			$others = array();
			$sqlfiles = glob(WSLM_ROOT.'/data/bakup/*.sql');
			if(is_array($sqlfiles))
			{
				...
				{
					if(preg_match("/(wslmtables_[0-9]{8}_[0-9a-z]{4}_)([0-9]+)\.sql/i",basename($sqlfile),$num))
					{
