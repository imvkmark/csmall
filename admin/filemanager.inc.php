...
include_once WSLM_ROOT.'images/ext/ext.php';
...
if(!isset($forward)) $forward = '?mod=wslm&file=database&action=main';
...
switch($action)
{
	...
	case 'uploadfile':
		...
		{
			$upfile_size = '4000000';
			$savepath = str_replace(str_replace("\\","/",WSLM_ROOT),'',$currentdir)."/";
			$fileArr = array(
			...
			if(file_exists(WSLM_ROOT.'/'.$savepath.$fileArr['name']) && !isset($overfile)) showmessage($LANG['find_same_file']);
			$savepath = str_replace(str_replace("\\", "/", WSLM_ROOT), "", $currentdir)."/";
		...
		if($dosubmit)
		{
			...
			for($i=0; $i<$filecount; $i++)
			{
				...
				if($source)
				{
					...
					if(file_exists(WSLM_ROOT.'/'.$savepath.$fileArr['name']) && !isset($overfile)) continue;
				}
			}
	...
	case 'phpinfo':
		...
		$phpinfo = preg_replace("/<a href=\"http:\/\/www.php.net\/\"><img(.*)alt=\"PHP Logo\" \/><\/a><h1 class=\"p\">(.*)<\/h1>/","<h1 style=\"text-align:center;font-family:Arial;color=red;\">\\2<h3>&nbsp;&nbsp;provide impetus for wslm!</h3></h1>",$phpinfo);
