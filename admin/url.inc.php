...
if($dosubmit)
{
	...
	if($pages > $page)
	{
		...
		$forward = $start ? "?mod=wslm&file=url&type=$type&dosubmit=1&firest=$firest&action=$action&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page=$page&pages=$pages&total=$total", URL);;
	}
	else
	{
		...
		if($WSLM['ishtml'])
		{
			...
			$forward = '?mod=wslm&file=html&action=show';
		}
		else
		{
			$message = "URL更新完成！";
			$forward = '?mod=wslm&file=url';
		}
	}
