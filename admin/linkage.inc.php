...
switch($action)
{
    case 'add':
		...
		{
			header("Location: ?mod=wslm&file=linkage&action=manage");
		}
		break;
		//添加子菜单
	 case 'addsub':
    ...
    case 'manage_submenu':
		...
		$areas = array();
				...
				$str = "<tr>
							...
							<td style='text-align:center'><a href='?mod=wslm&file=linkage&action=manage_submenu&keyid=\$keyid&linkageid=\$id#addsubmunu'>添加子菜单</a> | <a href='?mod=\$mod&file=\$file&action=edit&linkageid=\$id&parentid=\$parentid&keyid=\$keyid'>".$LANG['edit']."</a> | <a href=javascript:confirmurl('?mod=\$mod&file=\$file&action=delete&linkageid=\$id&keyid=\$keyid','确认要删除该菜单吗？')>".$LANG['delete']."</a></td></tr>";
				$tree->tree($areas);
    ...
    case 'delete':
		...
		if($result)
		{
			$forward = $keyid ? '?mod=wslm&file=linkage&action=manage_submenu&keyid='.$keyid : '?mod=wslm&file=linkage&action=manage';
			showmessage('操作成功！', $forward);
		}
		else
		{
