...
switch ($action)
{
	...
	case 'scan_file_md5':
		...
		$file_md5 = file(WSLM_ROOT.'data/md5_file/'.$safe['md5_file']);
	...
	case 'scan_func':
		...
		if($safe['func'])
		{
			...
			{
				$html = file_get_contents(WSLM_ROOT.$key);
	...
	case 'scan_code':
		...
		if ($safe['code'])
		{
			foreach ($file_list as $key=>$val)
			{
	            ...
	            if(strtolower(substr($key, -4)) == '.php' && function_exists('zend_loader_file_encoded') && zend_loader_file_encoded(WSLM_ROOT.$key))
	            {
	...
	case 'see_code':
		...
		$file_list = cache_read('scan_backdoor.php');
		$html = file_get_contents(WSLM_ROOT.$file_path);
	...
	case 'del_file':
		...
		@unlink(WSLM_ROOT.$file_path);
		showmessage('文件删除成功！', '?mod=wslm&file=safe&action=scan_table');
	break;
	...
	case 'replace':
		if (isset($dosubmit)) 
		{
			...
			foreach ($file_list as $key=>$val)
			{
				file_put_contents(WSLM_ROOT.$val, str_replace($html, $replace, file_get_contents(WSLM_ROOT.$val)));
			}
			showmessage('替换成功！', '?mod=wslm&file=safe&action=scan_table');
		}
		else 
		{
