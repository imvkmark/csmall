...
switch($action)
{
    case 'add':
		...
		if($dosubmit)
		{
			...
			$info['status'] = ($status == 2 || $status == 3) ? $status : ($allow_manage ? ($WSLM['publish'] && (strtotime($info['inputtime']) > TIME) ? 98 : 99)  : 3);
			...
			if($contentid) showmessage('发布成功！', '?mod=wslm&file=content&action=add&catid='.$catid);
		}
		else
		{
			$data['catid'] = $catid;
	...
	case 'block':
		...
		include admin_template('wslm', $template);
	...
	case 'category':
		...
		else
		{
			...
			if($page == 0)
			{
				...
				$categorys = $child ? subcat('wslm', $catid, 0) : array();
			}
			else
			{
