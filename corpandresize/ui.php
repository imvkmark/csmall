...
$tmp = WSLM_ROOT.str_replace($WSLM['siteurl'],'',$dirnames).'/';
$tmp_url = str_replace($WSLM['siteurl'],'',$dirnames);

if(preg_match("/http:/",$tmp))
{
	$tmp = WSLM_ROOT.UPLOAD_URL.date('Y').'/'.date('md').'/';
...
<table border="0" cellpadding="0" cellspacing="0" align="center" style="display:" id="coordlayer">
	...
	<tr>
		<td>
			...
			<input type="hidden" name="wslm_path" value="<?=WSLM_PATH?>" id="wslm_path" />
