...
class model
{
	...
	function add($model)
	{
		...
		$sql = file_get_contents(WSLM_ROOT.'include/admin/model.sql');
	...
	function delete($modelid)
	{
		...
		$this->cache();
		$C = load('category.class.php','wslm','include/admin/');
	...
	function import($array)
	{
		...
		$sql = file_get_contents(WSLM_ROOT.'include/admin/import_model.sql');
		$sql = str_replace($arr_search, $arr_replace, $sql);
		sql_execute($sql);
		return $modelid;
	}
	...
	function cache()
	{
		...
		cache_common();
		$fields = array();
		$files = glob(WSLM_ROOT.'include/fields/*');
	...
	function cache_class($fields, $classname)
	{
		...
		{
			$r = @file_get_contents(WSLM_ROOT.'include/fields/'.$field.'/'.$classname.'.inc.php');
			if($r) $data .= $r;
		}
		$classfile = 'content_'.$classname.'.class.php';
		$classcode = @file_get_contents(WSLM_ROOT.'include/fields/'.$classfile);
