...
class content
{
	...
	function edit($contentid, $data)
	{
		...
		{
			$prefix = $data['prefix'] ? $data['prefix'] : ($WSLM['enable_urlencode'] ? hash_string($contentid) : '');
			...
			if($data)
			{
				...
				if($this->db->affected_rows())
				{
					...
					if(!is_object($attachment))
					{
						...
						$attachment = new attachment('wslm', $data['catid']);
					}
					...
					{
						$this->db->query("DELETE FROM `".DB_PRE."comment` WHERE keyid='wslm-content-title-$contentid'");
					}
			...
			foreach($contentid as $id)
			{
				if($status==99 && isset($MODULE['pay']))
				{
					...
					if($catid && $presentpoint)
					{
						...
						$pay_api->update_exchange('wslm', 'point', $presentpoint, $api_msg, $cinfo['userid']);
					}
				}
