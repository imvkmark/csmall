...
class category
{
	...
	function __construct($module = 'wslm')
	{
		global $db, $CATEGORY;
		$this->db = &$db;
	...
	function add($category, $setting = array())
	{
		...
		if($this->module == 'wslm' && $category['type'] < 2)
		{
			$parentid = $category['parentid'];
			$this->menu->update('catid_'.$catid, array('parentid'=>$this->menu->menuid('catid_'.$parentid), 'name'=>$category['catname'], 'url'=>'?mod=wslm&file=content&action=manage&catid='.$catid));
	...
	function edit($catid, $category, $setting = array())
	{
		...
		if($this->module == 'wslm' && $category['type'] < 2)
		{
            $url = $this->category[$catid]['child'] ? '' : '?mod=wslm&file=content&action=manage&catid='.$catid;
	...
	function delete($catid)
	{
		...
		if($this->category[$catid]['type'] == 0)
		{
			...
			foreach($catids as $id)
			{
				...
				if($this->module == 'wslm' && $this->category[$id]['type'] < 2) $this->menu->update('catid_'.$id);
		...
		if($arrparentid)
		{
			...
			foreach($arrparentids as $id)
			{
				...
				if($this->module == 'wslm' && $this->category[$id]['type'] < 2) $this->menu->update('catid_'.$id, array('isfolder'=>$child));
			}
		}
		$this->cache();
		return true;
	...
	function repair()
	{
		...
		if(is_array($this->category))
		{
			foreach($this->category as $catid => $cat)
			{
				...
				if($cat['module']=='wslm' && $cat['type']!=2) $this->url($catid);
			}
		}
		$this->cache();
		return TRUE;
