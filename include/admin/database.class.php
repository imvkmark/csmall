...
class database
{
	...
	function export($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype)
	{
		...
		if(trim($tabledump))
		{
			$tabledump = "# wslm bakfile\n# version:".WSLM_VERSION."\n# time:".date('Y-m-d H:i:s')."\n# type:wslm\n# wslm:http://www.wslm.cn\n# --------------------------------------------------------\n\n\n".$tabledump;
			$tableid = $i;
			$filename = $tabletype.'_'.date('Ymd').'_'.$random.'_'.$fileid.'.sql';
			$altid = $fileid;
			$fileid++;
			$bakfile = WSLM_ROOT.'data/bakup/'.$filename;
			if(!is_writable(WSLM_ROOT.'/data/bakup/')) showmessage($this->lang['data_cannot_bak_to_server'], $forward);
		...
		else
		{
		   ...
		   showmessage($this->lang['database_bak_success'],'?mod=wslm&file=database&action=export');
		}
	}
	...
	function import($filename)
	{
		...
		{
			$filepath = WSLM_ROOT.'data/bakup/'.$filename;
		...
		else
		{
			...
			{
				showmessage($this->lang['database_recover_success'],'?mod=wslm&file=database&action=import');
			}
		}
	}
	...
	function repair($tables,$operation)
	{
		...
		if($tables && in_array($operation,array('repair','optimize')))
		{
			...
			showmessage($this->lang['operation_success'],'?mod=wslm&file=database&action=repair');
		}
		else
		{
			showmessage($this->lang['select_rep_table'],'?mod=wslm&file=database&action=repair');
		}
	}
	...
	function executesql($operation,$sql)
	{
		...
		if($operation == 'file')
		{
	        require_once WSLM_ROOT.'include/upload.class.php';
			$savepath = 'data/bakup/';
		...
		if($filenames)
		{
			if(is_array($filenames))
			{
				foreach($filenames as $filename)
				{
					...
					{
						$str = file_get_contents(WSLM_ROOT.'data/bakup/'.$filename);
						$str = str_charset($from, $to, $str);
						file_put_contents(WSLM_ROOT.'data/bakup/'.$to.$filename, $str);
						@chmod(WSLM_ROOT.'data/bakup/'.$to.$filename, 0777);
					}
				}
			}
			else
			{
				...
				{
					$str = file_get_contents(WSLM_ROOT.'data/bakup/'.$filenames);
					$str = str_charset($from, $to, $str);
					file_put_contents(WSLM_ROOT.'data/bakup/'.$to.$filenames, $str);
					@chmod(WSLM_ROOT.'data/bakup/'.$to.$filenames, 0777);
				}
			}
		...
		if($filenames)
		{
			if(is_array($filenames))
			{
				foreach($filenames as $filename)
				{
					if(fileext($filename)=='sql')
					{
						@unlink(WSLM_ROOT.'data/bakup/'.$filename);
					}
				}
			...
			else
			{
				if(fileext($filenames)=='sql')
				{
					@unlink(WSLM_ROOT.'data/bakup/'.$filenames);
					showmessage($this->lang['operation_success'], '?mod='.$this->mod.'&file='.$this->file.'&action='.$action);
				}
			}
		}
	...
	function down($filename)
	{
		...
		file_down(WSLM_ROOT.'data/bakup/'.$filename);
	...
	function status()
	{
		...
		return array('wslmtables'=>$wslm, 'othertables'=>$other);
	}
}
?>
