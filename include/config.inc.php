...
define('DB_PW', ''); //数据库密码
define('DB_NAME', 'mg_wlsm'); //数据库名
define('DB_PRE', 'wslm_'); //数据库表前缀，同一数据库安装多套Wslm时，请修改表前缀
...
// 网站路径配置
define('WSLM_BUG', 1);
define( 'WSLM_PATH'        , '/wslm/' ); // Main框架访问路径，相对于域名
define( 'JS_PATH'          , WSLM_PATH . 'statics/js/' );
define( 'CSS_PATH'         , WSLM_PATH . 'statics/css/' );

//shtml 支持
define('SHTML', 0); //是否支持 shtml，需要服务器支持，并且生成文件扩展名为 shtml

//数据存文本目录
define('CONTENT_ROOT', WSLM_ROOT.'data/txt/'); //默认存储路径

//缓存配置
define('CACHE_STORAGE', 'files'); //Cache 存储方式（files, mysql, apc, eaccelerator, memcache, shmop）
define('CACHE_PATH', WSLM_ROOT.'data/cache/'); //缓存默认存储路径
define('CACHE_MODEL_PATH', WSLM_ROOT.'data/cache_model/'); //模型缓存存储路径

//页面缓存配置
define('CACHE_PAGE', 0); //是否开启PHP页面自动缓存功能
define('CACHE_PAGE_PATH', WSLM_ROOT.'data/cache_page/'); //缓存存储路径
...
define('SESSION_SAVEPATH', WSLM_ROOT.'/data/sessions/'); //Session 保存路径（files）
...
define('COOKIE_PRE', 'zQKDaeAafb'); //Cookie 前缀，同一域名下安装多套Wslm时，请修改Cookie前缀
define('COOKIE_TTL', 0); //Cookie 生命周期，0 表示随浏览器进程

//模板相关配置
define('TPL_ROOT', WSLM_ROOT.'templates/'); //模板保存物理路径
define('TPL_NAME', 'default'); //当前模板方案目录
define('TPL_CSS', 'default'); //当前样式目录
define('TPL_CACHEPATH', WSLM_ROOT.'data/cache_template/'); //模板缓存物理路径
define('TPL_REFRESH', 1); //是否开启模板缓存自动刷新

//附件相关配置
define('UPLOAD_FRONT', 1); //是否允许前台上传附件
define('UPLOAD_ROOT', WSLM_ROOT.'uploadfile/'); //附件保存物理路径
...
define('UPLOAD_FTP_DOMAIN', 'http://upload.wslm.cn/'); //Ftp附件默认域名
...
define('ERRORLOG', 1); //是否保存错误日志
define('FILTER_ENABLE', 1); //非法信息屏蔽作用范围（0 禁用，1 前台，2 全站）
define('GZIP', 0); //是否Gzip压缩后输出
define('AUTH_KEY', 'hUyiTZHGZEnyiCqAIvoi'); //Cookie密钥
define('PASSWORD_KEY', 'asdfsfsdfsdfsd'); //会员密码密钥，为了加强密码强度防止暴力破解，不可更改
