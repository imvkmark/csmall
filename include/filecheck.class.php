...
class filecheck
{
    ...
    {
		$this->dir = WSLM_ROOT.'data/md5_file/';
    }
	...
	function set($filename = '')
	{
		...
		{
			$this->file = WSLM_ROOT.'data/md5_file/'.date('Y-m-d', TIME);
		}
		else
		{
	...
	function check($dir, $exts = 'php|js|html', $require = 1)
	{
		...
		{
			$md5 = md5_file($v);
			$filepath = str_replace(WSLM_ROOT, '', $v);
			$line = $md5.' '.$filepath."\n";
	...
	function make($dir, $exts = 'php|js|html')
	{
		...
		foreach($list as $v)
		{
			$data .= md5_file($v).' '.str_replace(WSLM_ROOT, '', $v)."\n";
		}
	...
	function dirs()
	{
		...
		foreach($list as $v)
		{
			if(is_dir($v)) $dirs[] = str_replace(WSLM_ROOT, '', $v);
		}
		return $dirs;
	}
	...
	{
		if($dir == './')$dir = '';
		$fp = dir(WSLM_ROOT.$dir);
		while (false !== ($en = $fp->read())) {
			if ($en != '.' && $en != '..') {
				if (is_dir(WSLM_ROOT.$dir.'/'.$en) && !empty($dir)) {
				...
				else 
				{
					if(!empty($option))
					{
						$p = pathinfo(WSLM_ROOT.$dir.'/'.$en, PATHINFO_EXTENSION);
						if (in_array($p, $option)) {
							$file_list[$dir.'/'.$en] = md5_file(WSLM_ROOT.$dir.'/'.$en);
						}
					}
					else 
					{
