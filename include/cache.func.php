...
function cache_common()
{
	...
	{
		if(!$r['path']) $r['path'] = $r['module'] == 'wslm' ? '' : $r['module'].'/';
		if(!$r['url']) $r['url'] = $r['module'] == 'wslm' ? '' : $r['module'].'/';
		$data[$r['module']] = $r;
	}
	...
	$CACHE['URLRULE'] = $data;
	$data = array();
    $r = $db->get_one("SELECT `setting` FROM `".DB_PRE."module` WHERE `module`='wslm'");
	$setting = $r['setting'];
	eval("\$WSLM = $setting;");
	if($WSLM['siteurl'] =='') $WSLM['siteurl'] = SITE_URL;
	$CACHE['WSLM'] = $WSLM;
