...
{
	global $WSLM;
	$filter_word = trim($WSLM['filter_word']);
	...
	if($pattern && preg_match("/($pattern)/", $data, $m))
	{
		...
		$logfile = WSLM_ROOT.'data/filterlog/'.date('Ym', TIME).'.csv';
		$fp = fopen($logfile, 'a');
		fputcsv($fp, $logdata);
		fclose($fp);
		return true;
...
function url($url, $isabs = 0)
{
	...
	{
		$url = strpos($url, WSLM_PATH) === 0 ? SITE_URL.substr($url, strlen(WSLM_PATH)) : SITE_URL.$url;
	}
	else
	{
		$url = strpos($url, WSLM_PATH) === 0 ? $url : WSLM_PATH.$url;
	}
	return $url;
}
...
function subcat($module = 'wslm', $parentid = NULL, $type = NULL)
{
	global $CATEGORY;
	$subcat = array();
...
function subtype($module = 'wslm', $modelid = 0)
{
	global $TYPE;
	$subtype = array();
...
function template($module = 'wslm', $template = 'index', $istag = 0)
{
...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'images/nopic_small.gif', $ftp = 0)
{
	...
	if(strpos($imgurl, '://')) {
		...
		if(file_exists(WSLM_ROOT.$newimgurl)) return $newimgurl;
		@dir_create(UPLOAD_ROOT.date('Y/md/'));
		@copy($imgurl, WSLM_ROOT.UPLOAD_URL.basename($imgurl));
		$imgurl = UPLOAD_URL.basename($imgurl);
		$ftp = 1;
	} else {
		if(!file_exists(WSLM_ROOT.$imgurl)) return 'images/nopic.gif';
		list($width_t, $height_t, $type, $attr) = getimagesize(WSLM_ROOT.$imgurl);
	...
	return $image->thumb(WSLM_ROOT.$imgurl, WSLM_ROOT.$newimgurl, $width, $height, '', $autocut, $ftp) ? $newimgurl : $imgurl;

}

function ssi($file)
{
	if(!file_exists(WSLM_ROOT.$file)) return false;
	return (SHTML && defined('CREATEHTML')) ? '<!--#include virtual="'.WSLM_PATH.$file.'"-->' : @file_get_contents(WSLM_ROOT.$file);
}
...
function get($sql, $rows = 0, $page = 0, $dbname = '', $dbsource = '', $urlrule = '', $distinct_field = '', $catid = 0)
{
	...
	{
		global $db;
		if(DB_PRE != 'wslm_') $sql = str_replace('wslm_', DB_PRE, $sql);
	}
...
{
	global $db, $CATEGORY, $MODULE, $URLRULE, $WSLM, $MODEL;
	if($sql)
	{
		@include_once WSLM_ROOT.$MODULE[$module]['path'].'include/output.func.php';
		$offset = 0;
		if($page !== 0)
		{
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(), $catid = 0)
{
	...
	else
	{
		...
		$data = str_replace('"', '\"', $WSLM['pageshtml']);
		eval("\$url = \"$data\";");
		return $url;
	}
}
...
function showmessage($msg, $url_forward = 'goback', $ms = 1250, $direct = 0)
{
	...
	include defined('IN_ADMIN') ? WSLM_ROOT.'admin/templates/showmessage.tpl.php' : template('wslm','showmessage');
	exit;
}
...
function keyid_get($keyid)
{
	...
	if($module=='video')
	{
		...
		require_once WSLM_ROOT.'video/include/output.func.php';
...
function tpl_data($module = 'wslm', $template = 'index')
{
...
function load($file, $module = 'wslm', $dir = '', $isinit = 1)
{
	global $MODULE;
	if(!isset($MODULE[$module])) return false;
	$path = WSLM_ROOT.$MODULE[$module]['path'].($dir ? $dir.'/' : 'include/').$file;
...
function wslm_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	$key_length = 4;
	$key = md5($key != '' ? $key : $GLOBALS['wslm_auth_key']);
	$fixedkey = md5($key);
...
function magic_image($txt, $fonttype = 4)
{
	...
	{
		$txt = urlencode(wslm_auth($txt, 'ENCODE', AUTH_KEY));
		$txt = '<img src="'.WSLM_PATH.'magic_image.php?gd=1&fonttype='.$fonttype.'&txt='.$txt.'" align="absmiddle">';
	}
	return $txt;
}
...
function wslm_error($errno, $errmsg, $filename, $linenum, $vars)
{
	$filename = str_replace(WSLM_ROOT, '.', $filename);
	...
	$err .= "</errorentry>\n\n";
	error_log($err, 3, WSLM_ROOT.'/data/php_error_log.xml');
	@chmod(WSLM_ROOT.'/data/php_error_log.xml', 0777);
}
...
function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0)
{
	...
	$datas = array();
	$datas = @include WSLM_ROOT.'data/linkage/'.$linkageid.'.php';
	$infos = $datas['data'];
	if($action=='edit')
	{
