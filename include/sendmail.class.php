...
class sendmail
{
	...
	{
		global $WSLM;
		$this->set($WSLM['mail_server'], $WSLM['mail_port'], $WSLM['mail_user'], $WSLM['mail_password'], $WSLM['mail_type']);
		$this->auth = 1;
	}
	...
	function send($email_to, $email_subject, $email_message, $email_from = '')
	{
		...
		$adminemail = $this->type == 1 ? $WSLM['mail_user'] : $WSLM['adminemail'];
		$email_from = $email_from == '' ? '=?'.CHARSET.'?B?'.base64_encode($WSLM['sitename'])."?= <$adminemail>" : (preg_match('/^(.+?) \<(.+?)\>$/',$email_from, $from) ? '=?'.CHARSET.'?B?'.base64_encode($from[1])."?= <$from[2]>" : $email_from);
		...
		$headers = "From: $email_from{$this->delimiter}X-Priority: 3{$this->delimiter}X-Mailer: Wslm {$this->delimiter}MIME-Version: 1.0{$this->delimiter}Content-type: text/html; charset=".CHARSET."{$this->delimiter}";
		if($this->type == 1)
		{
	...
	function smtp($email_to, $email_subject, $email_message, $email_from = '', $headers = '')
	{
		...
		fputs($fp, ($this->auth ? 'EHLO' : 'HELO')." Wslm\r\n");
		$lastmessage = fgets($fp, 512);
