...
class html
{
	...
	{
		global $WSLM;
		if(!$WSLM['ishtml']) return true;
		extract($GLOBALS, EXTR_SKIP);
		$head['title'] = $WSLM['sitename'].'-'.$WSLM['meta_title'];
		$head['keywords'] = $WSLM['meta_keywords'];
		$head['description'] = $WSLM['meta_description'];
		$subcats = subcat('wslm', 0, 0);
		$catid = 0;
		ob_start();
		include template('wslm', 'index');
		$file = WSLM_ROOT.$this->url->index();
	...
	function category($catid, $page = 0)
	{
        ...
        $head['title'] = $catname.'-'.($meta_title ? $meta_title : $WSLM['sitename']);
	...
	function show($contentid, $is_update_related = 0)
	{
		...
		{
			$this->index();
			$pages = intval($WSLM['autoupdatelist']);
		...
		unset($info);
		$show_url_path = str_replace('.'.$WSLM['fileext'],'',$show_url_path);
		...
		if(strpos($content, '[page]') !== false)
		{
			...
			foreach($contents as $content)
			{
				...
				$head['title'] = $title.'_'.$C['catname'].'_'.$WSLM['sitename'];
				ob_start();
				include template('wslm', $template);
				$file = WSLM_ROOT.$pageurls[$page][0];
