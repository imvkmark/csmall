...
class block
{
	...
	function refresh($pageid = '', $blockno = 0)
	{
		...
		foreach($array as $blockid=>$r)
		{
			...
			createhtml(WSLM_ROOT.'data/block/'.$blockid.'.html');
	...
	function set_html($blockid)
	{
		...
		$result = createhtml(WSLM_ROOT.'data/block/'.$blockid.'.html');
	...
	{
		$blockid = intval($blockid);
		return @file_get_contents(WSLM_ROOT.'data/block/'.$blockid.'.html');
	}

	function rm_html($blockid)
	{
		return @unlink(WSLM_ROOT.'data/block/'.$blockid.'.html');
	}
	...
	function set_html_blockno($pageid, $blockno)
	{
		...
		$file = WSLM_ROOT.'data/block/'.$pageid.'_'.$blockno.'.html';
	...
	{
		return @file_put_contents(WSLM_ROOT.'data/block/'.$pageid.'_'.$blockno.'.html');
	}

	function get_template_path($blockid)
	{
		return TPL_ROOT.TPL_NAME.'/wslm/block/'.$blockid.'.html';
	}
	...
	{
		$path = TPL_ROOT.TPL_NAME.'/wslm/block/example/';
