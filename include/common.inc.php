<?php
define('WSLM_ROOT', str_replace("\\", '/', substr(dirname(__FILE__), 0, -7)));
define('MICROTIME_START', microtime());
define('IN_WSLM', TRUE);
define('MAGIC_QUOTES_GPC', get_magic_quotes_gpc());
define('TIME', time());
set_include_path(WSLM_ROOT.'include/');
...
require WSLM_ROOT.'languages/'.LANG.'/wslm.lang.php';

ERRORLOG ? set_error_handler('wslm_error') : error_reporting(E_ERROR | E_WARNING | E_PARSE);
define('IP', ip());
...
define('SITE_URL', SCHEME.$_SERVER['HTTP_HOST'].WSLM_PATH);
...
define('CACHE_FORM', WSLM_ROOT.'data/formguide/');
...
if($WSLM['enable_ipbanned'] && ip_banned(IP)) showmessage($LANG['administrator_banned_this_IP']);
if(!defined('IN_ADMIN'))
{
	if(FILTER_ENABLE && filter_word()) showmessage('The content including illegal information: '.ILLEGAL_WORD.' .');
    if($WSLM['minrefreshtime'])
	{
		$cc = new times();
		$cc->set('cc', $WSLM['minrefreshtime'], 1);
		if($cc->check()) showmessage('Do not refresh the page in '.$WSLM['minrefreshtime'].' seconds!');
		$cc->add();
		unset($cc);
	}
...
$_username = '';
$_groupid = 3;
$wslm_auth = get_cookie('auth');
if($wslm_auth)
{
	$auth_key = md5(AUTH_KEY.$_SERVER['HTTP_USER_AGENT']);
	list($_userid, $_password) = explode("\t", wslm_auth($wslm_auth, 'DECODE', $auth_key));
	...
	unset($r, $wslm_auth, $wslm_auth_key, $_password, $sql_member);
}
$G = cache_read('member_group_'.$_groupid.'.php');
$priv_group = new priv_group();
define('SKIN_PATH', 'templates/'.TPL_NAME.'/skins/'.TPL_CSS.'/');
define('PASSPORT_ENABLE', ($WSLM['uc'] || $WSLM['enablepassport'] || $WSLM['enableserverpassport']) ? 1 : 0);
