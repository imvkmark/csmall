...
$isthumb = $WSLM['thumb_enable'] && $info['isthumb'] ? 1 : 0;
$iswatermark = $WSLM['watermark_enable'] && $info['iswatermark'] ? 1 : 0;
$thumb_width = $info['thumb_width'] ? $info['thumb_width'] : $WSLM['thumb_width'];
$thumb_height = $info['thumb_height'] ? $info['thumb_height'] : $WSLM['thumb_height'];
...
if ($dosubmit)
{
	...
	if($isthumb || $iswatermark)
	{
		...
		if($iswatermark) $image->watermark($img, '', $WSLM['watermark_pos'], $watermark_img, '', 5, '#ff0000', $WSLM['watermark_jpgquality']);
	}
