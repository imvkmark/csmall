...
<table cellpadding="0" cellspacing="1" class="table_form">
    ...
    <tr>
        ...
        <td>
        	<?=form::select_template($mod, 'info[template]', 'template', 'form_index', '', 'form_')?> &nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=wslm&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'">&nbsp;<input type="button" value="添加模板" title="添加模板" onClick="location.href='?mod=wslm&file=template&action=add&module=<?=$mod?>&forward=<?=urlencode(URL)?>'">
        </td>
    </tr>
    <tr>
    	<th></th>
