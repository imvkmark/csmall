...
if(!defined('MOD_ROOT'))
{
	define('MOD_ROOT', WSLM_ROOT.''.$MODULE['formguide']['url'].'/');
}
...
class formguide_admin extends formguide
{
	...
	function add($info, $setting)
	{
		...
		$sql = file_get_contents(WSLM_ROOT.'formguide/admin/include/formguide.sql');
	...
	function cache()
	{
		...
		cache_formguid();
		$fields = array();
		$files = glob(WSLM_ROOT.'formguide/admin/include/fields/*');
	...
	function cache_class($fields, $classname)
	{
		...
		{
			$r = @file_get_contents(WSLM_ROOT.'formguide/admin/include/fields/'.$field.'/'.$classname.'.inc.php');
			if($r) $data .= $r;
		}
		$classfile = 'formguide_'.$classname.'.class.php';
		$classcode = @file_get_contents(WSLM_ROOT.'formguide/admin/include/fields/'.$classfile);
