...
{
	$registerurl = $WSLM['passport_serverurl'].$WSLM['passport_registerurl'];
	...
	case 'register':
			if($dosubmit)
			{
				...
				if($M['enablemailcheck'])
				{
					...
					$title = $WSLM['sitename'].$LANG['member_register_email_verify'];
					$authurl = url($M['url'], 1)."register.php?action=activate&userid=$userid&authcode=$authstr";
					$content = tpl_data($mod, 'register_mailcheck');
					$sendmail->send($memberinfo['email'], $title, stripslashes($content), $WSLM['mail_user']);
				}
				...
				if($WSLM['enablepassport'])
	            {
					...
					{
						$info = str_charset(CHARSET, $WSLM['passport_charset'], $memberinfo);
					}
			...
			{			
				...
				$head['title'] = '新用户注册_'.$WSLM['sitename'];
