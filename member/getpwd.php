...
{
	$getpasswordurl = $WSLM['passport_serverurl'].$WSLM['passport_getpasswordurl'];
 			...
 			if($dosubmit)
 			{
				...
				if($step == 2 &&  $M['enableQchk'])
				{
 				...
 				if($step == 3)
 				{
					...
					$title = $WSLM['sitename'].$LANG['get_password'];
					...
					$sendmail->send($email,$title, stripslashes($content), $WSLM['mail_user']);
