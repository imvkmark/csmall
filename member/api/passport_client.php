<?php	
	...
	elseif($verify != md5($action.$userdb.$forward.$WSLM['passport_serverkey']))
	{
		exit('Illegal request');
	}
	$db_hash = $WSLM['passport_serverkey'];
	$_db_hash = $db_hash;
	...
	if ($action == 'login')
	{
		...
		elseif($WSLM['passport_expire'] && TIME - $remoteinfo['time'] > $WSLM['passport_expire'])
		{
		...
		$forward = empty($forward) ? $WSLM['passport_serverurl'] : $forward;
		$cookietime = $arr_member['cookietime'] ? TIME + $arr_member['cookietime'] : 0;
		$wslm_auth_key = md5(AUTH_KEY.$_SERVER['HTTP_USER_AGENT']);
		$wslm_auth = wslm_auth($userid."\t".$info['password'], 'ENCODE', $wslm_auth_key);
		set_cookie('auth', $wslm_auth, $cookietime);
	...
	elseif ($action == 'delete')
	{
		...
		if(!$forward) $forward = HTTP_REFERER ? HTTP_REFERER : $WSLM['passport_serverurl'];
