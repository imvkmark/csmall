...
elseif ($action == 'login')
{
	...
	if($uid==-1){
		...
		list($uid, $username, $uc_password, $email) =  uc_call("uc_user_login", array($wslmUsername, $wslmPassword));
	}
	if( $uid == -1 )
	{
		showmessage('用户不存在,或者被删除');
	...
	$wslm_userid = $member->get_userid($username);
	if($wslm_userid<=0 && $uid > 0)
	{
		require_once MOD_ROOT.'api/member_api.class.php';
		$member_api = new member_api();
	...
	}else if($wslm_userid>0 && $uid > 0){
		require_once MOD_ROOT.'api/member_api.class.php';
		$member_api = new member_api();
		$arr_member=array();
		$arr_member['touserid'] = $uid;//modify by skyz
		$member_api->set($wslm_userid,$arr_member);
	}
}
