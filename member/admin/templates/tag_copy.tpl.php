...
?>
    ...
    <tr> 
      ...
      <td><?=form::select_template($module, 'tag_config[template]', 'template', $template, '', 'tag_')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=wslm&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'"></td>
    </tr>
    <tr> 
...
&nbsp;
<input type="submit" name="preview" class="button_style" value=" 预览 " onClick="$('#action').val('preview');$('#mod').val('wslm');">
     </td>
    </tr>
  </form>
</table>
