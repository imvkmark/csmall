...
switch($action)
{
    case 'add':
		...
		{
			if($WSLM['uc']) showmessage('已经开启Ucenter整合，请到Ucenter处添加用户', $WSLM['uc_api']);
			unset($GROUP['1']);
    ...
    case 'manage':
		...
		$pagesize = $WSLM['pagesize'] ? $WSLM['pagesize'] : 30;
		...
		if($modelid && $issearch)
		{
			...
			$data = $member_search->data($page, $WSLM['search_pagesize']);
			foreach($data as $v)
			{
				$arr_userid[] = $v['userid'];
			}
	...
	case 'import_uc':
		if($dosubmit)
		{
			...
			else
			{
				...
				if(!$sqldb->connect($uc_dbhost, $uc_dbuser, $uc_dbpwd, $uc_dbname, 0, $uc_charset))
				{
					showmessage('连接不上服务器', '?mod=wslm&file=setting&tab=7');
				}
			}
			$dbpre = $uc_dbpre;
			$maxuid = getmaxuid();
			...
			$forward = $finished ? "?mod=wslm&file=setting&tab=7" : "?mod=$mod&file=$file&action=$action&name=$name&type=$type&offset=$newoffset&total=$total&uc_dbhost=$uc_dbhost&uc_dbuser=$uc_dbuser&uc_dbpwd=$uc_dbpwd&uc_charset=$uc_charset&uc_dbpre=$uc_dbpre&uc_dbname=$uc_dbname&uc_apiid=$uc_appid&dosubmit=1";
