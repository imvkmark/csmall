...
class mails
{
    ...
    function send($email, $typeids, $flag = 0)
    {
        ...
        $title = '订阅确认';
        $this->m->send($email, $title, $content,$WSLM['mail_user']);
        return true;
    }
...
//------游客订阅，退订处理
    ...
    {
        global $_userid, $_username, $_email, $WSLM;
        ...
        if($this->setTypeMail($email, $typeids))
        {
            if($email != $newemail)
            {
                ...
                if($this->db->query($sql))
                {
                    $this->m->send($newemail, $title, $content, $WSLM['mail_user']);
                    $this->error = 6;
                    return false;
                }
            }
