...
function template_compile($tplname, $template)
{
	...
	if($content === false)
	{
		...
		$MS['urls'][0] = array(
			'name'=>'商务管理中心重新选择企业模板',
			'url'=>$WSLM['siteurl'].'yp/'.$M['businessdir'].'/?',
			);
		$MS['urls'][1] = array(
			'name'=>'注册为本站会员',
			'url'=>$WSLM['siteurl'].'member/register.php',
			);
		msg($MS);
	}
	$compiledtplfile = TPL_CACHEPATH.'yp_'.$tplname.'_'.$template.'.tpl.php';
	$content = ($istag || substr($template, 0, 4) == 'tag_') ? '<?php function _tag_'.$module.'_'.$template.'($data, $number, $rows, $count, $page, $pages, $setting){ global $WSLM,$MODULE,$M,$CATEGORY,$TYPE,$AREA,$GROUP,$MODEL,$templateid,$_userid,$_username;@extract($setting);?>'.template_parse($content, 1).'<?php } ?>' : template_parse($content);
...
{
	$tplfile = TPL_ROOT.TPL_NAME.'/wslm/block/'.$blockid.'.html';
	$compiledtplfile = TPL_CACHEPATH.'wslm_block_'.$blockid.'.tpl.php';
...
function template_parse($str, $istag = 0)
{
	...
	if(!$istag) $str = "<?php defined('IN_WSLM') or exit('Access Denied'); ?>".$str;
	return $str;
}
