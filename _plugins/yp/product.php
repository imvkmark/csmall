...
switch($action)
{
	case 'show':
		...
		$head['description'] .= $rs['title'].'_产品'.'_'.$WSLM['sitename'];
		$head['title'] .= $rs['title'].'_产品'.'_'.$WSLM['sitename'];
	...
	case 'search':
		$head['keywords'] .= '产品搜索';
		$head['description'] .= '产品搜索'.'_'.$WSLM['sitename'];
		$head['title'] .= '产品搜索'.'_'.$WSLM['sitename'];
		$CAT = subcat('yp', 0);
	...
	case 'searchlist':
		...
		$head['description'] .= '产品搜索结果'.'_'.$WSLM['sitename'];
		$head['title'] .= '产品搜索结果'.'_'.$WSLM['sitename'];
		$page = $page?$page:1;
		$catid = intval($catid);
	...
	default:
		$catid = intval($catid);
		$head['keywords'] .= '产品';
		$head['description'] .= '产品'.'_'.$WSLM['sitename'];
		$head['title'] .= '产品'.'_'.$WSLM['sitename'];
		$CAT = subcat('yp', 0);
		if($catid)
		{
