...
switch($action)
{
	...
	case 'update':
		...
		if($dosubmit)
	    {
		    ...
		    include admin_tpl('tag_list','wslm');
		}
		else
	    {
			$message = '<form name="myform" method="post" action="?mod=wslm&file=tag&action=listtag&module='.$module.'&template='.$template.'&templatename='.$templatename.'&dosubmit=1"><input type="hidden" name="content"></form><script type="text/javascript">myform.content.value=window.opener.document.myform.content.value;myform.submit();</script>';
			showmessage($message);
		}
	break;
		...
		if(!isset($tag_config) && $t->get_tag($tagname))
		{
			...
			if(isset($info['catid']) || $tag_config['catid'] == '$catid')
			{
				...
				if($chk_var === 0)
				{
					...
					{
						include admin_tpl('tag_preview', 'wslm');
					}
				}
				else
				{
					include admin_tpl('tag_preview_form', 'wslm');
				}
			}
			else
			{
