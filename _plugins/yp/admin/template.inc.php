...
switch($action)
{
	case 'add':
		if($dosubmit)
		{
			$companytpl_config = include WSLM_ROOT.'templates/'.TPL_NAME.'/yp/companytplnames.php';
			...
			cache_write('companytplnames.php', $companytpl_config,WSLM_ROOT.'templates/'.TPL_NAME.'/yp/');
			showmessage('添加成功！');
		}
		else
		{
			...
			$templatelist = glob(WSLM_ROOT.'templates/'.TPL_NAME."/yp/com_*.html");
	...
	case 'manage':
		if($dosubmit)
		{
			$companytplnames = include WSLM_ROOT.'templates/'.TPL_NAME.'/yp/companytplnames.php';
		...
		{
			unset($GROUP[2],$GROUP[3]);
			$templatenames = include WSLM_ROOT.'templates/'.TPL_NAME.'/yp/name.inc.php';
			$companytpl_config = include WSLM_ROOT.'templates/'.TPL_NAME.'/yp/companytplnames.php';
			...
			foreach($companytpl_config AS $key=>$_v)
			{
				...
				$string .= " <input name='thumb$key' type='text' id='thumb$key' size='70' value='$_v[thumb]'>  <input type='button' value='上传缩略图' onclick=\"javascript:openwinx('?mod=wslm&file=upload&uploadtext=thumb$key&type=thumb&width=600&height=300','upload_thumb','350','350')\">";
				$string .= "</td></tr>";
