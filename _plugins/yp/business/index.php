...
if(!$company_user_infos)
{
	...
	$MS['urls'][0] = array(
		'name'=>'免费升级为企业会员',
		'url'=>$WSLM['siteurl'].$M['url'].'company.php?action=member',
		);
	$MS['urls'][1] = array(
		'name'=>'退出当前帐号，换其他帐号登陆',
		'url'=>$WSLM['siteurl'].'member/logout.php',
		);
	$MS['urls'][2] = array(
		'name'=>'重新注册为企业会员',
		'url'=>$WSLM['siteurl'].'member/logout.php?forward='.urlencode($WSLM['siteurl'].'member/register.php'),
		);
	msg($MS);
}
$CATEGORY = subcat('yp');
...
function check_priv($file)
{
	...
	if(!in_array($_groupid,$M["allow_add_$file"]))
	{
		...
		$MS['urls'][0] = array(
			'name'=>'升级会员组',
			'url'=>$WSLM['siteurl'].'member/upgrade.php',
			);
		$MS['urls'][1] = array(
			'name'=>'返回商务中心',
			'url'=>'?',
