function change_f_catid(catid)
{
	if(catid)
	{
		$('#secondcatid').empty();
		$('#thirdcatid').empty();
		$.get(wslm_path+'yp/load.php?action=cat_option&catid='+catid, function(data){
			if(data==1)
			{
				$('#hava_checked').val('1');
			}
...
function YP_checkform()
{
	...
	else
	{
		...
		if(secondcatid==null)
		{
			$.get(wslm_path+'yp/load.php?action=catid_select&catid='+catid, function(data){
				if(data == '1')
				{
					alert('请选择二级分类');
					$('#secondcatid').focus();
		...
		else
		{
			...
			if(thirdcatid==null)
			{
				$.get(wslm_path+'yp/load.php?action=catid_select&catid='+secondcatid, function(data){
					if(data == '1')
					{
						alert('请选择三级分类');
						$('#thirdcatid').focus();
