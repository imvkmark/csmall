...
?>
  <tr>
    ...
    <td style="text-align:center;"><a href="?mod=wslm&file=content&action=edit&catid=0&contentid=<?=$error['contentid']?>"><?=$error['error_title']?></a></td>
	...
	<td style="text-align:center;"><a href="?mod=wslm&file=content&action=edit&catid=0&contentid=<?=$error['contentid']?>">原文</a> | <? if ($error['status']){?>已审核<?  }else{ ?><a href="javascript:if(confirm('确定要审核该条记录吗？')) location='?mod=<?=$mod?>&file=<?=$file?>&action=check&errorid=<?=$error['error_id']?>'">未审核</a><? } ?> | <a href="javascript:if(confirm('确定删除该条记录吗？')) location='?mod=<?=$mod?>&file=<?=$file?>&action=delete&errorid=<?=$error['error_id']?>'" >删除</a></td>
  </tr>
<?
    }
?>
