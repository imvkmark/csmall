...
<table cellpadding="0" cellspacing="1" class="table_list">
 <caption>Digg 统计排行</caption>
  ...
  <tr>
    <td class="align_c"><?=form::select_category('wslm', 0, 'catid', '', '不限栏目', $catid, 'onchange="redirect(\'?mod=digg&file=range&range='.$range.'&catid=\'+this.value)"',1)?></td>
...
<?php
$result = get("select * from wslm_digg d left join wslm_content c on d.contentid=c.contentid where c.status=99 $where order by $orderby desc", 20, $page);
...
<?php
$data = get("select * from wslm_digg d left join wslm_content c on d.contentid=c.contentid where c.status=99 $where order by d.supports_day desc", 10);
foreach($data as $k=>$r)
{
?>
  <tr>
...
<?php
$data = get("select * from wslm_digg d left join wslm_content c on d.contentid=c.contentid where c.status=99 $where order by d.supports_week desc", 10);
foreach($data as $k=>$r)
{
?>
  <tr>
...
<?php
$data = get("select * from wslm_digg d left join wslm_content c on d.contentid=c.contentid where c.status=99 $where order by d.supports_month desc", 10);
foreach($data as $k=>$r)
{
?>
  <tr>
