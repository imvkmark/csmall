...
$html = "<link href=\"".$WSLM['siteurl'].SKIN_PATH."{$mod}.css\" rel=\"stylesheet\" type=\"text/css\" /><div id=\"digg\"><span><strong id=\"diggDivCount_{$contentid}\">{$dig_arr[1]}</strong><br />{$sup}</span><span class=\"cai\"><strong id=\"diggDivCount_down_{$contentid}\">{$dig_arr[2]}</strong><br /><samp id=\"diggDivDo_down_{$contentid}\" onclick=\"digg(0)\" >{$aga}</samp></span></div>";
?>
$("#digg_div").html("<?=addslashes($html)?>");
function digg(id)
{
	$("#digg_div").html('正在处理...');
	$("#calldigg_js").attr('src', "<?=$WSLM['siteurl']?><?=$MODULE['digg']['url']?>digg.php?contentid=" + contentid+"&catid="+catid+"&flag="+id);
}