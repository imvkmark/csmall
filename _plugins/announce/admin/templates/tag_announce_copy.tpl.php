...
?>
<body>
<script type="text/javascript" src="<?=WSLM_PATH?>include/js/tag.js"></script>
...
<table class="table_form" cellspacing="1" cellpadding="0">
   ...
   <input name="forward" type="hidden" value="<?=$forward?>"> 
    ...
    <tr> 
      <td><b>此标签调用的模板</b></td>
      <td>
<?=form::select_template('announce', 'tag_config[template]', 'template', $template, '','tag_announce')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=wslm&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'"> 【注:只能修改非默认模板】
      </td>
    </tr>
	<tr> 
