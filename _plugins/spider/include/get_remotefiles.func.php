...
if ($WSLM['watermark_enable'])
{
    require_once WSLM_ROOT.'/include/image.class.php';
	$watermark_img = WSLM_ROOT.$WSLM['watermark_img'];
	$image = new image();
}
...
{
    global $image,$watermark_img, $WSLM, $att;

    $filepath = WSLM_PATH . $uploaddir . '/' . date('Ym') . '/';
    $uploaddir = WSLM_ROOT . '/' . $uploaddir . '/' . date('Ym') . '/';
    ...
    foreach($files as $k => $file)
    {
        ...
        { 
            ...
            if (@copy($file, $newfile))
            {
                ...
                if ($WSLM['watermark_enable']&& file_exists($newfile))
                {
					$image->watermark($newfile, '', $WSLM['watermark_pos'], $watermark_img, '', 5, '#ff0000', $WSLM['watermark_jpgquality']);
                }
                @chmod($newfile, 0777);
                if (is_object($att)) $att->addfile(str_replace(WSLM_ROOT, '', $newfile));
            } 
        } 
        else
        {
