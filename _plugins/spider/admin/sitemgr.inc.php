...
{ 
    ...
    case 'delete':
        if (is_numeric($siteid))
        {
            ...
            @unlink(WSLM_ROOT . "/spider/rules/site_" . $siteid . ".php");
        } elseif (is_array($siteid))
        {
            ...
            foreach ($siteid as $sd)
            {
                @unlink(WSLM_ROOT . "/spider/rules/site_" . $sd . ".php");
            } 
        } 
    ...
    case 'catchsiteinfo':
        ...
        if (!file_exists(WSLM_ROOT . "/spider/rules/site_" . $siteid . ".php"))
            showmessage($LANG['cannot_continue_error_info_not_fount_rule_file'] . WSLM_ROOT . "/spider/rules/site_" . $siteid . ".php");
        $ruledata = file_get_contents(WSLM_ROOT . "/spider/rules/site_" . $siteid . ".php");
    ...
    case 'modify':
        if (isset($Submit))
        {
            ...
            array_save($rule, '\$rule', WSLM_ROOT . "/spider/rules/site_" . $siteid . ".php");
            showmessage($LANG['operation_success'], "?mod=" . $mod . "&file=sitemgr");
        } 
        ...
        @include (WSLM_ROOT . "/" . $mod . "/rules/site_" . $siteid . ".php");
    ...
    case 'add':
        if (isset($Submit))
        {
            ...
            { 
                ...
                $rule['HtmlTrim'] = $r;
                $rule = dstripslashes($rule);
                array_save($rule, '\$rule', WSLM_ROOT . "/spider/rules/site_" . $siteinsertid . ".php");
    ...
    case 'siteout':
        ...
        {
            $fileurl = WSLM_ROOT . "/data/temp/" . $LANG['out_site_name'] . ".psite";
        ...
        else
        {
            ...
            while ($row = $db->fetch_row($rows))
            {
                ...
                if (!file_exists(WSLM_ROOT . "/$mod/rules/site_" . $siteid . ".php"))
                    showmessage($LANG['cannot_continue_error_info_not_fount_rule_file'] . WSLM_ROOT . "/$mod/rules/site_" . $siteid . ".php");
                $ruledata = file_get_contents(WSLM_ROOT . "/$mod/rules/site_" . $siteid . ".php");
                $filecontent = "INSERT INTO [" . $LANG['site_database'] . "] VALUES(♂" . $tabledump . "♂" . $ruledata;
                $key = "wslm_locoy";
                $filecontent = auth_encrypt($filecontent, $key);

                @$fp = fopen(WSLM_ROOT . "/data/temp/" . $LANG['out_site_name'] . ".psite", "w");
                @flock($fp, 3);
    ...
    case 'sitein':
        if (isset($extract) && $extract == 1)
        {
            ...
            if (!file_exists(WSLM_ROOT . "/data/temp/sitein.psite"))
                showmessage($LANG['cannot_continue_load_job_rule']);
            $filecontent = auth_decrypt(file_get_contents(WSLM_ROOT . "/data/temp/sitein.psite"), "wslm_locoy");
            ...
            if ($insertid > 0)
            {
                @$fp = fopen(WSLM_ROOT . "/" . $mod . "/rules/site_" . $insertid . ".php", "w");
                @flock($fp, 3);
    ...
    case 'manage':
        ...
        $result = $db->query("SELECT * FROM " . TABLE_SPIDER_SITES . "  order by ID desc limit $offset," . $WSLM['pagesize']);
        $sites = array();
