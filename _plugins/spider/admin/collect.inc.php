...
require WSLM_ROOT . '/include/tree.class.php';
...
switch ($action)
{
    ...
    case 'outcontent':
        ...
        include (WSLM_ROOT . "/" . $mod . "/admin/job_parse.class.php");
        $CJob = new Job_Parse();
        $CJob->mod = $mod;
        ...
        if ($step)
        {
            ...
            $dbFields = include_once(WSLM_ROOT . '/data/cache_model/' . $modelid . '_fields.inc.php');
    ...
    case 'spideradd':
		...
		if ($submit)
        {
            ...
            $modelFields = include_once(WSLM_ROOT . '/data/cache_model/' . $modelid . '_fields.inc.php'); 
			...
			$attachment = new attachment('wslm', $commonPublish['catid']);
            $c = new content();
			...
			$publish['thumb'] = str_replace(WSLM_PATH.'uploadfile/', 'uploadfile/', $publish['thumb']);
                        ...
                        { 
                            ...
                            if (!empty($pictureurls))
                            {
                                ...
                                foreach($pictureurls as $url)
                                {
                                    ...
                                    $url = str_replace(substr(WSLM_PATH,1).'uploadfile/', '', $url);
                                    $sql = "insert into " . DB_PRE . "attachment (module,catid,contentid,field,filename,filepath,filetype,fileext,isimage,isthumb,userid,uploadtime,uploadip)values('wslm','" . $info['catid'] . "','$contentid','$field','$filename','$url','$filetype','$fileext',1,0,'" . $info['userid'] . "','$Now','127.0.0.1')";
                                    $db->query($sql);
                                } 
                            }
    ...
    case 'manage':
        ...
        $query .= " " . $sortby . " limit $offset," . $WSLM['pagesize'];
        // pre($query);
        $result = $db->query($query);
        $contents = array();
        while ($r = $db->fetch_array($result))
