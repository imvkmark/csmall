...
function showcat(keyid,catid)
{
    ...
    var pars = "mod=wslm&file=tag&action=category_select&catid="+catid+"&keyid="+keyid;
...
document.all.userlabel.innerHTML+="<div id=\"fu"+labelnum+"\"  style=\"display:block\"><table width='100%' align='center'><tr ><td background='<?=WSLM_PATH?>/images/spacer.gif'></td></tr></table>";
	setModelSelected();
}
...
<caption>网址采集</caption>
	...
	<?=form::select_category('wslm', 0, 'parentid', 'parentid', '无（作为一级栏目）', $job_CatId,$cat_js)?>&nbsp;<font color="Red">*</font></td>
	</tr>
	<tr>
...
通配符:<a href="javascript:AddOnPos(document.myform.jobDividePageEnd,'(*)');" style="color:#3300FF;">(*)</a>
	  ...
	  分页合并代码：<input type="text" name="job[DividePageUnion]" cols="26" rows="4" id="jobDividePageUnion" value="<?=$job_DividePageUnion?>">此处为分页采集后内容链接代码，[page]是wslm默认分页符，为空表示字符直接连接在一起。
	  </td>
    </tr> 
    <tr >
