...
switch ($action)
{
    case 'jobin':
        if (isset($extract) && $extract == 1)
        {
            ...
            include(WSLM_ROOT . "/" . $mod . "/admin/des.inc.php");
            if (!file_exists(WSLM_ROOT . "/data/temp/jobin.pjob"))
                showmessage($LANG['cannot_continue_to_load_job']);
            $filecontent = auth_decrypt(file_get_contents(WSLM_ROOT . "/data/temp/jobin.pjob"), "wslm_locoy");
            ...
            if ($insertid > 0)
            {
                @$fp = fopen(WSLM_ROOT . "/" . $mod . "/rules/" . $insertid . ".php", "w");
                @flock($fp, 3);
        ...
        else
        {
            if (isset($uploadjob))
            {
                include_once WSLM_ROOT . "/include/upload.class.php";
                $upfile_size = '1000000';
                $upfile_type = 'pjob';
    ...
    case 'catchinfo': 
        ...
        include(WSLM_ROOT . "/spider/admin/des.inc.php");
        ...
        if (!file_exists(WSLM_ROOT . "/spider/rules/" . $jobid . ".php"))
            showmessage($LANG['job_cannot_continue_info_not_found_job_rule'] . WSLM_ROOT . "/spider/rules/" . $jobid . ".php");
        $ruledata = file_get_contents(WSLM_ROOT . "/spider/rules/" . $jobid . ".php");
        $filecontent = "INSERT INTO [" . $LANG['job_database'] . "] VALUES(♂" . $tabledump . "♂" . $ruledata;
        $key = "wslm_locoy";
        $jobrulecontent = auth_encrypt($filecontent, $key);
        $usersite = $WSLM['siteurl'];
    ...
    case 'delete':
        ...
        if (!is_numeric($jobid))
        {
            ...
            {
                foreach($jobid as $jd)
                @unlink(WSLM_ROOT . "/spider/rules/" . $jd . ".php");
        ...
        else
        {
            ...
            if ($db->affected_rows() > 0)
            {
                @unlink(WSLM_ROOT . "/spider/rules/" . $jobid . ".php");
    ...
    case 'modify':
        ...
        if (isset($save))
        {
            ...
            array_save($rule, "\$rule", WSLM_ROOT . "/spider/rules/" . $jobid . ".php");
            showmessage($LANG['operation_success'], "?mod=" . $mod . "&file=" . $file . "&action=manage");
        } 
        else
        {
            ...
            @include (WSLM_ROOT . "/" . $mod . "/rules/" . $jobid . ".php");
            ...
            if (isset($job_CatId))
            {
                ...
                foreach($CATEGORY as $row)
                {
                    if ($row['catid'] == $loadModelId)
                    {
                        ...
                        $fs = include_once(WSLM_ROOT . '/data/cache_model/' . $row['modelid'] . '_fields.inc.php');
    ...
    case 'add':
        ...
        else
        {
            ...
            $labelkeys = array();
            if (isset($loadsiterule) && file_exists(WSLM_ROOT . "/" . $mod . "/rules/site_" . $loadsiterule . ".php"))
            {
                @include (WSLM_ROOT . "/" . $mod . "/rules/site_" . $loadsiterule . ".php");
                $labelkeys = array_keys($rule['LabelName']);
            } 
            else
            {
    ...
    case 'manage':
        ...
        $serverip = gethostbyname('www.wslm.cn');
        ...
        $offset = ($page-1) * $WSLM['pagesize'];
        $r = $db->get_one("SELECT count(*) as num FROM " . TABLE_SPIDER_JOB);
        $number = $r['num'];
        $pages = pages($number, $page, $WSLM['pagesize']);
        $result = $db->query("SELECT JobId,JobName,JobDescription,SiteId,CreateOn,UpdateOn," . TABLE_SPIDER_SITES . ".Id," . TABLE_SPIDER_SITES . ".SiteName as SiteName" . " FROM " . TABLE_SPIDER_JOB . "," . TABLE_SPIDER_SITES . " WHERE SiteId=" . TABLE_SPIDER_SITES . ".Id order by JobId desc limit $offset," . $WSLM['pagesize']);
        $jobs = array();
    ...
    case 'jobcopy':
        if (isset($copyjob))
        {
            ...
            if (!file_exists(WSLM_ROOT . "/spider/rules/" . $jobid . ".php"))
                showmessage($LANG['job_cannot_copy_info_not_found_job_rule'] . WSLM_ROOT . "/spider/rules/" . $jobid . ".php");
            $sql = "INSERT INTO " . TABLE_SPIDER_JOB . " VALUES('','" . $siteid . "'," . $tabledump;
            $db->query($sql);
            $insertid = $db->insert_id();
            if ($insertid > 0)
            {
                @$fp = copy(WSLM_ROOT . "/spider/rules/" . $jobid . ".php", WSLM_ROOT . "/spider/rules/" . $insertid . ".php");
    ...
    case 'jobout':
        ...
        {
            $fileurl = WSLM_ROOT . "/data/temp/" . $LANG['name_of_out_job'] . ".pjob";
        ...
        else
        {
            ...
            if (@!file_put_contents(WSLM_ROOT . '/data/temp/' . $LANG['name_of_out_job'] . '.pjob', $filecontent))
            {
