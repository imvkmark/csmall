...
$answer = new answer();
require_once WSLM_ROOT.'member/include/member.class.php';
$member = new member();
...
switch($action)
{
	case 'ask':
		...
		$pages = $ask->pages;
		$head['title'] = '提问管理_问吧_'.$WSLM['sitename'];
	...
	case 'answer':
		...
		$pages = $answer->pages;
		$head['title'] = '我的回答_问吧_'.$WSLM['sitename'];
	...
	case 'actor':
		...
		else
		{
		    ...
		    $head['title'] = '修改头衔_问吧_'.$WSLM['sitename'];
	...
	case 'credit':
		...
		$array = $member->get($_userid,'i.actortype',1);
		$actortype = $array['actortype'];
		$head['title'] = '查看积分_问吧_'.$WSLM['sitename'];
	...
	case 'edit':
		if($job=='ask')
		{
			...
			else
			{
		        ...
		        $head['title'] = '修改提问_问吧_'.$WSLM['sitename'];
		...
		else
		{
			...
			else
			{
				...
				extract($r);
		        $head['title'] = '修改回答_问吧_'.$WSLM['sitename'];
				include template('ask', 'center_edit');
			}
