...
<input name='submit' type='submit' value='留言搜索'></form>
  ...
  <div class="tag_menu">
    <ul>
      <li><a href="<?php echo $M['url'];?>index.php">留言本首页</a></li>
      <li class="selected"><a href="<?php echo $M['url'];?>post.php">签写留言</a></li>
    </ul>
  </div>

  <form name="myform" method="post" action="<?php echo $M['url'];?>post.php" onSubmit="return check()">
    <table cellspacing="3" cellpadding="3" class="table_form">
      <tbody>
<tr>
<th>&nbsp;头像选择：</th>
<td colspan="2"><?php echo $face;?></td>
</tr>
        <tr>
          <th><font color="red">*</font>&nbsp;姓　名：</th>
          <td colspan="2"><input name="guestbook[username]" type="text" size="25" value="<?php echo $_username;?>" require="true" datatype="require" msg="<font color='red'>姓名不能为空</font>"></td>
        </tr>
...
<td colspan="2"><input name="guestbook[title]" type="text" id="guestbook[title]" size="40" require="true" datatype="require" msg="<font color='red'>标题不能为空</font>"></td>
        ...
        <tr>
          <th><font color="red">*</font>&nbsp;内　容：<p>最多<strong id="num"><?php echo $M['maxcontent'];?></strong>个英文字母</p><br /><span id="contentmsg"></span></th>
<td  colspan="2">
...
<?php echo form::editor('content', 'basic', 550, 400);?> </td>
        </tr>
...
function checknum()
{
var num = <?php echo $M['maxcontent'];?>-FCKeditorAPI.GetInstance('content').GetXHTML().length;
$('#nmu').html(num);
}
