...
$rssid = intval($rssid);
$head['title'] ='RSS订阅中心'.' - '.$WSLM['sitename'];
$head['keywords'] = $M['name'];
$head['description']  = $M['name'].' - '.$WSLM['sitename'];
if(!empty($rssid))
{
    ...
    $about      =  SITE_URL;
    $title      =  $catname['catname'].'-'.$WSLM['sitename'];
    $description =  $WSLM['meta_description'];
    $image_link =  SITE_URL.'images/logo.gif';
    $category   =  $WSLM['sitename'];
    $cache      =  60;
...
else
{
    ...
    $categorys = subcat('wslm', $catid, 0);
    include template('wslm', 'rss');
}
?>