function SelectPic()
{
  var arr=Dialog('?mod=wslm&file=file_select&type=thumb','',700,500);
  if(arr!=null)
  {
    var s=arr.split('|');
    $('#thumb').val(s[0]);
...
{	
	var s=Dialog('?mod=wslm&file=keyword&action=select','',700,500);
	if(s!=null)
	{
		if($('#keywords').val() == '')
		{
...
{	
	var s=Dialog('?mod=wslm&file=author&action=select','',700,500);
	if(s!=null)
	{
		$('#author').val(s);
	}
...
{	
	var s=Dialog('?mod=wslm&file=copyfrom&action=select','',700,500);
	if(s!=null)
	{
		$('#copyfrom').val(s);
	}
