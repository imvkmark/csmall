...
{
	var arr=Dialog('?mod=wslm&file=file_select&catid='+catid+'&isimage='+isimage, '', 700, 500);
	if(arr!=null)
	{
		var s=arr.split('|');
		$('#'+textid).val(s[0]);
...
{
	var arr=Dialog('?mod=wslm&file=more_pic_select', '', 700, 500);
	if(arr!=null)
	{
		var select = '';
		$.get('?mod=wslm&file=more_pic_select&action=getdata&currentdir='+arr, function(data){
		    if(data !=null)
			{
