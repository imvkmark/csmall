...
function get_menu(id,obj,islen){
	...
	$('#position').load('?mod=wslm&file=menu&action=menu_pos&menuid='+id);
	...
	$.ajax({type:'get', url:'?mod=wslm&file=menu&action=get_menu_list&menuid='+id, cache:cache_refresh,dataType:'json', success:function(json){
	var htmls="";
	var isend ="tree_line";
	var open = new Array();
	...
	$.each(json,function(i,n){
		if (json.max != null && n.name != undefined)
		{
			...
			htmls += "<div class='tree_div' onclick=\""+click+"$('#position').load('?mod=wslm&file=menu&action=menu_pos&menuid="+n.menuid+"');\" id='tree_div_"+n.menuid+"'>";
			var width = islen*16;
