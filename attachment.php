...
switch($action)
{
    case 'upload':
		if($dosubmit)
	    {
			...
			if($from == 'fckeditor')
			{
				...
				if($WSLM['watermark_enable'])
				{
					...
					$image = new image();
					$image->watermark($imagefile, $imagefile, $WSLM['watermark_pos'], $WSLM['watermark_img']);
				}
				if($WSLM['thumb_enable']  && in_array($extension,array('jpg','jpeg','gif','png','bmp')))
				{			
					$imagefile = UPLOAD_ROOT.$filepath;
					$image = new image();
					$filename = $image->thumb($imagefile, $imagefile, $WSLM['thumb_width'], $WSLM['thumb_height']);			
				}
	    ...
	    {
			include template('wslm', 'attachment_upload');
		}
		break;
